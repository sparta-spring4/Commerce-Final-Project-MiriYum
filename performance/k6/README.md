...
- staging은 검토된 `staging-api.miriyum.click`만 신뢰 allowlist에 고정한다. 이 변경만으로 요청은 실행되지 않으며, 아래 staging gate의 HTTPS·승인·smoke 증적 조건을 모두 충족해야 한다.
...
staging smoke에는 `TARGET_ENV=staging`, HTTPS `BASE_URL`, `STAGING_APPROVED=true`를 전달한다. baseline에는 추가로 `PROFILE=staging-baseline`, `STAGING_SMOKE_RUN_ID`, `SMOKE_PROOF_PATH=/results/{STAGING_SMOKE_RUN_ID}.json`을 전달한다. 신뢰 staging host는 `staging-api.miriyum.click` 하나다. `https://staging-api.miriyum.click`의 health check와 팀 승인 전에는 실행하지 않는다. production hostname, 실사용자 계정 또는 운영 데이터는 어떤 값으로도 실행하지 않는다.

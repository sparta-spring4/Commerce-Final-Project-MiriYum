...
local `recovery`는 승인된 고정 arm delay를 사용한다. staging `recovery`는 fixed delay를 허용하지 않고 `SSE_RECOVERY_RENDEZVOUS_APPROVED=true`와 정확한 repository·Issue·GitHub Actions run ID·일회성 UUID를 요구한다. 준비된 workflow가 dispatch actor의 정확한 `SSE_RECOVERY_FIRE` 코멘트를 받은 뒤 가까운 미래의 Valkey 중단 epoch를 예약한다. 원격 preflight가 root-only readiness 파일에 정확한 epoch를 기록하고 별도의 고정 SSM probe가 이를 검증했으며 주 interruption 명령도 여전히 `InProgress`인 경우에만 workflow가 ARMED를 게시한다. probe 실패·직렬 실행·timeout 또는 주 명령 종료 시에는 marker와 Waiting mutation 없이 fail-closed한다. 하네스는 `github-actions[bot]`이 남긴 같은 scope의 `SSE_RECOVERY_ARMED` 코멘트만 공개 GitHub API에서 인증 header 없이 조회한다. staging에서는 READY와 ARMED를 먼저 확정하고 5~30초의 안전 lead를 확인한 뒤에만 유한 수명의 SSE 연결을 연다. 초기 changed frame 뒤 공유 epoch까지 기다려 mutation하므로 제어면 대기 시간이 연결 수명을 소모하지 않는다. marker가 없거나 scope·author·시각·lead가 맞지 않거나 초기 frame 전에 mutation epoch가 지나면 stream과 Waiting mutation 전에 실패한다. marker 본문과 rendezvous 식별자는 summary에 포함하지 않는다.

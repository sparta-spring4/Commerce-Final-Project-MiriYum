...
const STORE_ROUTE = fillPath(ROUTES.storeOperatorStore, { storeId: STORE_ID })
...
describe('매장 운영자 셸', () => {
  ...
  it('매장을 알면 운영 홈과 관리 항목을 함께 보여 준다', async () => {
    ...
    ).toHaveAttribute('href', ROUTES.storeOperatorHome)
    expect(screen.getByRole('link', { name: '메뉴 관리' })).toHaveAttribute(
      'href',
      fillPath(ROUTES.storeOperatorMenus, { storeId: STORE_ID }),
    )
  })
  ...
  it('매장을 모르면 매장별 항목 없이 운영 홈만 남긴다', async () => {
    ...
    renderShell(ROUTES.storeOperatorHome)

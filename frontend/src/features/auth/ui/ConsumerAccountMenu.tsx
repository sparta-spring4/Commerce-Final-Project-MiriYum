 ...
 * 계정 메뉴에 넣을 항목.
 *
 * `NAVIGATION.consumer`에서 가져온다. 여기서 따로 나열하면 route 표와 어긋나고
 * 나중에 화면이 늘 때 한쪽만 갱신된다. 매장 찾기는 로그인 여부와 무관한 공용
 * 항목이고 헤더 주 메뉴가 이미 들고 있어 중복이므로 뺀다.
 */
const LINKED_ITEMS = NAVIGATION.consumer.filter(
  (item) => item.path !== ROUTES.stores,
)
 ...
 */
const MENU_ITEMS = [
  ...LINKED_ITEMS.filter((item) => item.path === ROUTES.myPage),
  ...LINKED_ITEMS.filter((item) => item.path !== ROUTES.myPage),
]

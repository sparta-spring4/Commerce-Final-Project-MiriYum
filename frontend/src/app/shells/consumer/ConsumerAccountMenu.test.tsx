...
function renderMenu(route: string = PUBLIC_PATHS.home) {
  return render(
    <TestQueryProvider>
      <ConsumerAuthProvider>
        <MemoryRouter initialEntries={[route]}>
          ...
          <Routes>
            ...
            <Route path={CONSUMER_PATHS.myReservations} element={<LocationProbe />} />
          </Routes>
        </MemoryRouter>
      </ConsumerAuthProvider>
    </TestQueryProvider>,
...
describe('ConsumerAccountMenu', () => {
  ...
  it('공용 화면에서도 계정 메뉴로 마이페이지에 갈 수 있다', async () => {
    ...
    renderMenu(PUBLIC_PATHS.home)

    fireEvent.click(await accountButton())

    expect(
      screen.getByRole('link', { name: '마이페이지' }),
    ).toHaveAttribute('href', CONSUMER_PATHS.myPage)
  ...
  it('항목을 누르면 그 경로로 이동하고 패널을 닫는다', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId('location')).toHaveTextContent(CONSUMER_PATHS.myPage),
    )
    expect(screen.queryByRole('link', { name: '마이페이지' })).toBeNull()
  })
  ...
  it('로그아웃하면 홈으로 보낸다', async () => {
    ...
    renderMenu(CONSUMER_PATHS.myPage)

    fireEvent.click(await accountButton())
    fireEvent.click(screen.getByRole('button', { name: '로그아웃' }))

    await waitFor(() =>
      expect(screen.getByTestId('location')).toHaveTextContent(PUBLIC_PATHS.home),
    )
  })

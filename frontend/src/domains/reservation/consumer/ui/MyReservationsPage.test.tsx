...
} from '../../../account/consumer/profile/test/fixtures'
...
function renderList(route: string = CONSUMER_PATHS.myReservations) {
    ...
    <QueryClientProvider client={queryClient}>
      <ConsumerAuthProvider>
        <MemoryRouter initialEntries={[route]}>
          <Routes>
            <Route
              path={CONSUMER_PATHS.myReservations}
...
describe('내 예약 내역', () => {
  ...
  it('전체 필터는 status를 보내지 않는다', async () => {
    ...
    renderList(`${CONSUMER_PATHS.myReservations}?status=CANCELLED`)
    await screen.findByRole('link', { name: '파스타 마스터즈' })
  ...
  it('노쇼 상태를 필터로 전달하고 negative 배지로 표시한다', async () => {
    ...
    renderList(`${CONSUMER_PATHS.myReservations}?status=NO_SHOW`)
  ...
  it('계약에 없는 상태값은 무시하고 전체로 조회한다', async () => {
    ...
    renderList(`${CONSUMER_PATHS.myReservations}?status=WAITING`)
  ...
  it('페이지는 0 기반으로 보낸다', async () => {
    ...
    renderList(`${CONSUMER_PATHS.myReservations}?page=2`)

...
import { LocationProbe } from '../../../../store/store-operator/test/renderOperator'
import { StoreOperatorSignInPage } from './StoreOperatorSignInPage'

function renderSignInAt(route: string = STORE_OPERATOR_PATHS.signIn) {
  return render(
    <StoreOperatorAuthProvider>
      <MemoryRouter initialEntries={[route]}>
        <Routes>
          <Route
            path={STORE_OPERATOR_PATHS.signIn}
            element={<StoreOperatorSignInPage />}
          />
          <Route path={STORE_OPERATOR_PATHS.home} element={<LocationProbe />} />
          <Route
            path={STORE_OPERATOR_PATHS.operatingHours}
...
describe('식당 대표자 로그인 화면', () => {
  ...
  it('보존한 목적지로 복귀한다', async () => {
    ...
    renderSignInAt(
      `${STORE_OPERATOR_PATHS.signIn}?returnTo=%2Fstore-operator%2Fstores%2F7%2Foperating-hours`,
    )
    await screen.findByLabelText('이메일')
  ...
  it('외부 오리진 목적지는 무시하고 운영 홈으로 보낸다', async () => {
    ...
    renderSignInAt(
      `${STORE_OPERATOR_PATHS.signIn}?returnTo=https%3A%2F%2Fevil.example%2Fsteal`,
    )
    await screen.findByLabelText('이메일')
  ...
  it('계정 유형을 고르는 입력 없이 셸별 진입점만 제공한다', async () => {
    ...
    ).toHaveAttribute('href', CONSUMER_PATHS.signIn)

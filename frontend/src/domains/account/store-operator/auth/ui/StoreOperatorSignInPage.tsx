...
export function StoreOperatorSignInPage() {
  ...
  const destination = readReturnTo(location.search) ?? STORE_OPERATOR_PATHS.home
          ...
          <div className="op-auth__panel">
            ...
            >
              ...
              <p className="op-auth__switch">
                대표자 계정이 없으신가요?{' '}
                <Link to={STORE_OPERATOR_PATHS.signUp}>대표자 회원가입</Link>
              </p>
              <p className="op-auth__switch">
                {/* 일반 사용자와 셸이 다르다. 같은 폼에서 역할을 고르게 하지 않는다. */}
                <Link to={CONSUMER_PATHS.signIn}>일반 사용자 로그인</Link>
              </p>

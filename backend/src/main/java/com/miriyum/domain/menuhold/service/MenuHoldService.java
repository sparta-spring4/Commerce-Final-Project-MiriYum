...
/**
 * 일반 예약 조정자가 메뉴 홀드 생성·해제·이행 완료·몰수에 사용하는 공개 계약이다.
 *
 ...
 * 호출한다. 생성·해제·이행 완료·몰수의 각 논리 작업에는 외부 Idempotency-Key와 다른 전역 고유
...
public interface MenuHoldService {
    ...
    /**
     * 예약 종결 조정자가 멱등 결과와 예약 aggregate를 먼저 잠근 뒤, 수용량 처리 전에 연결 메뉴 홀드
    ...
    /**
     * 예약 노쇼 시 수량 복구 없이 홀드를 몰수 상태로 종결한다. 몰수 operationId는 다른 논리
     * 작업의 operationId와 달라야 한다.
     */
    @Transactional(propagation = Propagation.MANDATORY)
    MenuHoldCommandResult forfeit(MenuHoldForfeitCommand command);
}

 ...
 * 취소 조정자는 예약 aggregate를 잠근 뒤 수용량 버킷보다 먼저 {@link #lockForTermination}을
 * 호출하고, 수용량 처리 뒤 연결 홀드가 있을 때만 {@link #release}를 호출한다. 생성·해제·이행
 * 완료의 각 논리 작업에는 외부 Idempotency-Key와 다른 전역 고유 operationId를 발급하며,
 * 다른 예약·메뉴 집합·수량·명령 종류에 재사용하지 않는다. 멱등 결과 replay에서는 이 서비스를
 * 다시 호출하지 않는다.</p>
 *
...
public interface MenuHoldService {
    ...
    /**
     * 예약 취소의 수용량 처리 전에 연결 메뉴 홀드 루트 행만 잠근다. 홀드 상태를 해석하거나
     * 상태·메뉴 수량·원장을 변경하지 않는다.
     *

...
public class OnboardingReviewCommandService {
    ...
    public ReviewCaseDetail assign(
            PlatformOperatorPrincipal principal, String caseId, AssignmentRequest request,
            String approval, String correlationId, IdempotencyKey key) {
        ...
        audit.appendOnboarding(new OnboardingEvent(
                ...
                PlatformOperatorAuditOutcome.SUCCESS, key.value()));
        return after;
    }

    @Transactional
    public ReviewCaseDetail reassign(
            PlatformOperatorPrincipal principal, String caseId, ReassignmentRequest request,
            String approval, String correlationId, IdempotencyKey key) {
        ReviewCaseDetail before = workflow.getReviewCase(caseId);
        ...
        assignments.close(assignment(caseId, request.expectedCaseVersion(), principal.accountId()));
        assignments.assign(new AdminCaseAssignmentCommand(
                AdminCaseType.ONBOARDING_REVIEW, caseId, after.caseVersion(),
                request.nextOperatorId(), expiresAt));
        audit.appendOnboarding(new OnboardingEvent(
    ...
    @Transactional
    public ApplicationData decide(
            PlatformOperatorPrincipal principal, String caseId, DecisionRequest request,
            String approval, String correlationId, IdempotencyKey key) {
        ...
        ApplicationData result = workflow.decide(new ReviewDecisionCommand(
                ...
                request.action(), request.reasonCode(), new ReviewActorContext(context.operatorId()),
                key.value()));
        assignments.close(assignment(caseId, request.expectedCaseVersion(), principal.accountId()));
        audit.appendOnboarding(new OnboardingEvent(

...
public class StoreSanctionCommandService {
 ...
 public IdempotentOutcome create(IdempotencyCommand command,PlatformOperatorPrincipal principal,long storeId,String caseId,long caseVersion,
                            SanctionCreate request,String correlationId){return idempotency.execute(command,()->{
  ...
  append(principal,PlatformOperatorAuditAction.STORE_SANCTION_CREATED,storeId,caseId,caseVersion,command.idempotencyKey(),correlationId);
 ...
 public IdempotentOutcome approve(IdempotencyCommand command,PlatformOperatorPrincipal principal,long storeId,String caseId,long caseVersion,long sanctionId,
                             SanctionApproval request,String approvalToken,String correlationId){return idempotency.execute(command,()->{
  StoreSanctionCase c=cases.requireAssigned(principal,storeId,caseId,caseVersion);
  ...
  audit.appendStore(new PlatformOperatorAuditWriter.StoreEvent(context.operatorId(),context.authorityVersion(),context.roles(),context.permissions(),PlatformOperatorAuditAction.STORE_SANCTION_APPROVED,String.valueOf(storeId),caseId,caseVersion,command.idempotencyKey(),correlationId));
 ...
 public IdempotentOutcome release(IdempotencyCommand command,PlatformOperatorPrincipal principal,long storeId,String caseId,long caseVersion,long sanctionId,
                             SanctionRelease request,String approvalToken,String correlationId){return idempotency.execute(command,()->{
  ...
  audit.appendStore(new PlatformOperatorAuditWriter.StoreEvent(context.operatorId(),context.authorityVersion(),context.roles(),context.permissions(),PlatformOperatorAuditAction.STORE_SANCTION_RELEASED,String.valueOf(storeId),caseId,caseVersion,command.idempotencyKey(),correlationId));return success(HttpStatus.OK,s.getId(),s.data());});
 }

...
public class ReservationScheduleEntry {
    ...
    static ReservationScheduleEntry from(WeeklyInterval interval) {
        if (interval.kind() != ScheduleIntervalKind.RESERVATION_SLOT) {
            throw new IllegalArgumentException("operating interval is not a reservation entry");
        }
        ReservationScheduleEntry entry = new ReservationScheduleEntry();

 ...
 * 멱등 실행 대상 명령의 식별 정보다.
 *
 * @param principalNamespace 인증 주체 namespace({@code consumer} 또는 {@code store-operator})
...
public record IdempotencyCommand(
    ...
    private static final Set<String> PRINCIPAL_NAMESPACES =
            Set.of("consumer", "store-operator");
    ...
    public IdempotencyCommand {
        ...
        if (!PRINCIPAL_NAMESPACES.contains(principalNamespace)) {
            throw new IllegalArgumentException(
                    "principalNamespace는 consumer 또는 store-operator여야 합니다.");
        }
        if (principalId <= 0) {

...
class MenuImageControllerTest {
    ...
    @Test
    void upsertMenuImageReturnsPublicUrl() throws Exception {
        UUID imageId = UUID.fromString("123e4567-e89b-12d3-a456-426614174000");
        given(publicImageService.upsertMenuImage(eq(11L), eq(7L), eq(21L), any(IdempotencyKey.class), any()))
                .willReturn(new PublicImageCommandResult(200,
                        new PublicImageResponse(imageId, "/api/v1/public-files/" + imageId)));

        MockMultipartFile image = new MockMultipartFile(
                "file", "menu.png", "image/png",
                new byte[] {(byte) 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A});

        mockMvc.perform(multipart("/api/v1/store-operators/stores/7/menus/21/image")
                        .file(image)
                        .with(request -> {

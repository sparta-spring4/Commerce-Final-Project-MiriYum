...
class MemberAppealServiceTest {
    @Test
    void reductionKeepsAccountSuspendedWhenAnotherActiveSuspensionRemains() {
        ...
        when(sanctions.countOtherActiveSuspensions(
                MemberAccountType.CONSUMER, 41, 5, LocalDateTime.now(clock))).thenReturn(1L);
        ...
        when(port.advanceSupportVersion(41, 4)).thenReturn(5L);
        ...
        verify(port, never()).clearSuspension(41, 4);
    }

...
class StoreRecommendationServiceTest {
    ...
    void combinesSearchAvailabilitySignalsAndHistoryIntoTheRealRanking() {
        RecommendationSearchSignals condition = condition();
        List<RecommendationSearchCandidate> candidates = List.of(
                new RecommendationSearchCandidate(
                        1L, 4, RecommendationAvailability.NOT_REQUESTED, null),
                new RecommendationSearchCandidate(
                        2L, 4, RecommendationAvailability.NOT_REQUESTED, null));
    ...
    void missingStoreSignalExcludesOnlyThatCandidate() {
        ...
        List<RecommendationSearchCandidate> candidates = List.of(
                new RecommendationSearchCandidate(
                        1L, 2, RecommendationAvailability.AVAILABLE, null),
                new RecommendationSearchCandidate(
                        2L, 2, RecommendationAvailability.AVAILABLE, null));
    ...
    private static RecommendationSearchSignals condition() {
        return new RecommendationSearchSignals(List.of(), List.of(), List.of());
    }
}

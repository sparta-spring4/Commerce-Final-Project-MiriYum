...
- OpenAI 요청이 잔여 표현과 승인 모델·차원만 포함하고, Qdrant payload가 메뉴·매장·버전 ID 외 원문·개인정보를 포함하지 않는지 계약 테스트한다.
- Qdrant 의미 후보의 stale 게시 버전·비공개 메뉴·폐점 매장이 MySQL 재검증에서 제거되는지 실제 MySQL로 검증한다.
- OpenAI·Qdrant timeout·오류·형식 불일치에 정확 MySQL 검색이 유지되고, 메뉴 커밋 실패에는 색인이 실행되지 않으며 색인 실패에는 메뉴 쓰기가 롤백되지 않는지 검증한다.
- 외부 호출 성공률·지연, 정확 검색 폴백, 색인 실패·재시도와 인덱스 지연을 지표로 관측한다.

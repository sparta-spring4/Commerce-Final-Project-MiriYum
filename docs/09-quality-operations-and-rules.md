...
- `AudienceOpenApiContractTest`는 기능별 OpenAPI 원본을 자동 탐색해 public·consumer·store-operator path 집합의 무중복 분할과 전체 노출을 검증한다. 1차 MVP aggregate와 명시된 이후 단계 audience path의 합집합은 전체 audience path와 일치해야 하며, 이후 단계 path는 aggregate에 포함될 수 없다. 진입점에는 구 URL이 없고 모든 path item은 단일 `$ref`여야 한다.

...
[#120](https://github.com/sparta-spring4/Commerce-Final-Project-MiriYum/issues/120)의 ECR·SSM 백엔드 API 사전 배포는 `staging` EC2만 대상으로 하며 프론트엔드 사용자 shell과 최종 same-origin 배포를 대체하지 않는다. 이 경로는 ECR 이미지의 SHA 추적, 비밀의 서버 분리, SSM 배포, Docker 실행과 loopback health 확인만 증명한다. `dev`에 통합된 뒤 CI가 성공한 SHA만 staging에 배포한다. 실제 `dev` 배포 실행·ECR push·SSM command·EC2 health의 성공 증거가 없으면 각각 `NOT CONFIGURED` 또는 `NOT RUN`으로 기록하며, 프론트엔드 소유자가 `/` 정적 제공과 `/api` 프록시를 포함한 후속 범위를 승인할 때까지 핵심 사용자 흐름 배포 성공으로 선언하지 않는다. 운영 배포는 이 경로와 분리해 staging과 별도인 ECS Fargate·ALB·RDS·Valkey, OIDC IAM 역할과 GitHub `production` Environment 승인으로 구성한다. 운영 CD workflow는 GitHub 기본 브랜치 `dev`에 등록하되 `main`의 성공한 Backend CI SHA만 대상으로 한다. `production` Environment는 workflow run ref인 `dev`만 허용하고 수동 실행도 `dev` ref로 제한한다. 이는 deployment branch 확인과 이미지 source 확인을 분리하기 위한 경계이며, 실제 image SHA는 별도 main 이력·exact CI 성공 검증을 통과해야 한다. `PRODUCTION_ECS_DEPLOYMENT_ENABLED=true`, Environment 보호 규칙, ECR `IMMUTABLE` 태그 정책이 모두 준비되기 전에는 deploy job을 실행하지 않는다.

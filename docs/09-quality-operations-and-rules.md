...
- `integrationTest` task는 `@Tag("integration")` 테스트만 실행하며, `test` task는 이 태그를 제외한다. `build`는 두 task를 모두 포함한다.
- `Backend CI`는 unit과 integration job을 병렬 실행하고, 둘 다 성공한 뒤에만 required check 이름인 `backend-ci`를 성공 처리한다. 새 통합 테스트가 태그를 빠뜨리면 Gradle 검증 task가 실패한다.

...
> 브랜치: `feature/32-mysql-idempotency` (base `1a2e175`)
...
- **Flyway 순서:** PR #40(#31) 먼저 병합 → #32가 최신 dev rebase 후 `V2026_07_29_03` 사용. #32는 #40보다 먼저 병합하지 않는다. out-of-order 미사용.
...
db/migration/V2026_07_29_03__create_idempotency_commands.sql
```
...
3. **Migration** `V2026_07_29_03__create_idempotency_commands.sql` — E-007 `idempotency_command_id` 대리 PK + 업무 복합 UNIQUE, 식별 컬럼 `as_cs`, result_* nullable, `TEXT` payload.
4. **의존성** — production `spring-boot-flyway`, test `testcontainers-junit-jupiter`/`testcontainers-mysql`(#40 병합 후 rebase 시 대부분 해소).
...
- [ ] 표준 UUID 검증·누락·형식 오류 `COMMON_003/004` → `IdempotencyKeyTest`.
- [ ] `(namespace,id,command_type,key)` 유일 제약 선점 → migration + IT.
- [ ] 같은 지문 재요청 재생 → IT replay.
- [ ] 다른 지문 키 재사용 `COMMON_007`, 기존 미변경 → IT conflict.
- [ ] 멱등 기록+업무 변경이 호출 도메인 같은 트랜잭션 참여 → executor `MANDATORY` + IT.
- [ ] 자동 만료·TTL·Valkey 없음 → 코드·의존성 부재로 확인.
...
- #40 병합 전 #32 병합 금지, 적용된 Flyway 파일 수정 금지.

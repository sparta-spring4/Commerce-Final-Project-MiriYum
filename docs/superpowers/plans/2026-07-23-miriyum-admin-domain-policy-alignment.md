...
> **에이전트 작업자용:** 필수 하위 스킬: `superpowers:subagent-driven-development`(권장) 또는 `superpowers:executing-plans`를 사용하여 이 계획을 작업별로 구현한다. 단계 추적에는 체크박스(`- [ ]`) 문법을 사용한다.

**목표:** 관리자 권한·승인 절차와 결제, 매장 상태, 개인정보, 입점·테이스팅 심사 정본 사이의 충돌을 없애고, 이미 확정된 정책 상태와 TODO 경계는 그대로 보존한다.

**아키텍처:** `15-admin-operation.md`는 관리자 역할, 사건 큐, 제안·승인·실행 절차를 소유한다. 각 도메인 문서는 실제 업무 객체의 상태 전이와 금전·개인정보 효과를 소유한다. 관리자 문서가 도메인 규칙을 우회하지 못하도록 교차 참조와 적용 우선순위를 명시하고, 같은 개념의 이름이 다른 경우에는 도메인 상태를 바꾸지 않는 관리자 큐 투영표를 둔다.

**기술 스택:** Markdown, PowerShell, ripgrep(`rg`), Git 읽기 전용 검증
...
- [ ] **단계 2: `운영자 공통 불변식`에 정책 소유권과 적용 우선순위를 추가한다**
  ...
  예상: 관리자 권한 보유만으로 도메인 전이나 금전·개인정보 규칙을 우회할 수 없다는 문장이 명시된다.

- [ ] **단계 3: 결정 기록을 추가한다**

  `ADMIN-003·ADMIN-005·ADMIN-006·ADMIN-007·ADMIN-009·ADMIN-010·ADMIN-011` 교차 도메인 정합성 행을 `2026-07-23` 날짜로 하나 추가한다.

  예상: 상태는 기존과 같고, 변경 사유는 “정본 소유권과 적용 우선순위 명시”로 기록된다.

### 작업 2: 일반 환불·보상과 금전 영향 수동 복구의 승인 경계를 분리한다

**파일:**

- 수정: `docs/service-policies/08-payment-refund.md`의 `PAY-015 운영자 수동 복구와 승인` 아래
- 수정: `docs/service-policies/15-admin-operation.md`의 `ADMIN-005` 및 `ADMIN-006` 아래
...
- [ ] **단계 1: `PAY-015`에 작업 분류와 승인 우선순위를 명시한다**
  ...
  예상: `PAY-015`만 읽어도 10만 원 수동 보정은 2인 승인이며, 10만 원 정상 정책 환불은 조건 충족 시 1인 승인임을 구분할 수 있다.

- [ ] **단계 2: `ADMIN-005`에서 잘못된 금액 한도 상속 표현을 제거한다**

  금전 복구에 “`ADMIN-006` 금액 임계값을 적용한다”는 모호한 문장을 다음으로 대체한다.

  - 금전 효과 수동 복구는 금액과 관계없이 이중 승인한다.
  - `ADMIN-006`은 일반 환불·별도 보상의 금액 경계를 제공한다.
  - 수동 복구가 원 결제금액 초과 별도 보상까지 포함하면 두 정책의 승인 조건을 모두 충족한다.

  예상: 20만 원 이하라는 이유로 금전 영향 수동 복구를 한 명이 제안·승인할 수 없다.

- [ ] **단계 3: `ADMIN-006`의 적용 범위를 일반 환불과 별도 보상으로 한정한다**

  도입 범위 문장과 명시적인 `PAY-015` 예외를 추가한다.

  - 이 한도는 정상 정책 흐름의 환불과 별도 보상 승인에 적용한다.
  - 원장 불일치·부분 실패·수동 보정은 `PAY-015`와 `ADMIN-005`의 이중 승인 규칙을 우선한다.

- [ ] **단계 4: 양쪽 문서에 같은 인수 예시를 추가한다**
...
- [ ] **단계 5: 결정 기록을 동기화한다**
  ...
  두 파일에 `2026-07-23` 행을 추가한다. `PAY-015`, `ADMIN-005`, `ADMIN-006` 상태는 변경하지 않는다.
...
- [ ] **단계 6: 결제 경계를 검증한다**
  ...
  예상: 일반 환불의 금액 기준과 금전 영향 수동 복구의 무조건 이중 승인이 두 문서에서 같은 의미로 나타난다.

### 작업 3: 정상 폐점·임시 제재·영구 퇴점의 전이를 분리한다

**파일:**

- 수정: `docs/service-policies/03-store-operation.md`의 `OPER-009` 아래
- 수정: `docs/service-policies/15-admin-operation.md`의 `ADMIN-007` 아래
...
- [ ] **단계 1: `OPER-009`에서 운영자의 직접 영구 제재 우회 가능성을 제거한다**

  인가된 운영자가 정책 위반에 폐점을 적용하도록 허용하는 광범위한 규칙을 다음으로 대체한다.
...
- [ ] **단계 2: `ADMIN-007`에 도메인 실행 경계를 추가한다**
...
- [ ] **단계 4: 결정 기록을 동기화하고 검증한다**
  ...
  예상: `OPER-009`만으로 정책 위반 영구 퇴점을 실행할 수 있다는 문장이 남지 않는다.

### 작업 4: 개인정보 내보내기·break-glass·감사 TODO 경계를 일치시킨다

**파일:**

- 수정: `docs/service-policies/15-admin-operation.md`의 `운영자 공통 불변식`, `ADMIN-009`, `ADMIN-010`, `ADMIN-011` 아래
- 수정: `docs/service-policies/17-privacy-security.md`의 `PRIV-011` 아래
- 보존: `docs/service-policies/17-privacy-security.md`의 기존 사용자 변경
...
- [ ] **단계 1: 절대 금지 문구를 승인 없는 작업 금지로 좁힌다**

  `운영자 공통 불변식`에서 “대량 내보내기를 허용하지 않는다”를 취지에 맞게 다음으로 대체한다.

  - 무승인·자유 형식 대량 내보내기를 허용하지 않는다.
  - 승인된 대량 내보내기는 `PRIV-011`과 `ADMIN-010`을 모두 따라야 한다.
  - 비밀 조회, 직접 DB 수정, 공용 계정 사용은 계속 금지한다.

- [ ] **단계 2: `ADMIN-010`에 개인정보 대량 작업 조건을 명시한다**
...
- [ ] **단계 3: `ADMIN-011`의 개인정보 break-glass에 다른 승인자를 요구한다**
...
- [ ] **단계 5: `PRIV-011`을 같은 조건으로 맞춘다**

  개인정보 도메인을 소유자로 보존하고 교차 참조를 추가한다.
...
- [ ] **단계 6: 양쪽 결정 기록을 동기화한다**

  `2026-07-23` 행을 추가하되 `ADMIN-009`를 TODO에서, `PRIV-011`을 확정에서 변경하지 않는다.
...
### 작업 5: 관리자 심사 큐와 도메인 심사 상태의 투영 관계를 명시한다

**파일:**

- 수정: `docs/service-policies/15-admin-operation.md`의 `ADMIN-003` 아래
- 최소 수정: `docs/service-policies/02-store-onboarding.md`의 `STORE-005`와 공통 심사 상태 설명 아래
- 최소 수정: `docs/service-policies/07-course-tasting.md`의 `TASTE-001` 아래
...
- [ ] **단계 1: `ADMIN-003`에 투영표와 불변식을 추가한다**

  위 매핑 표를 추가하고 다음을 명시한다.
...
- [ ] **단계 2: `02-store-onboarding.md`에는 역참조 한 문단만 추가한다**
  ...
  전체 매핑 표를 중복하거나 기존 상태의 이름을 바꾸지 않는다.

- [ ] **단계 3: `07-course-tasting.md`에는 역참조 한 문단만 추가한다**
  ...
  코스/테이스팅 제품 정책 또는 MVP 범위를 확장하지 않는다.

- [ ] **단계 4: 세 문서의 결정 기록을 동기화한다**

  상태 투영과 소유권 명확화만 설명하는 `2026-07-23` 행을 추가한다. 모든 정책 상태는 변경하지 않는다.
...
### 작업 6: 정책 템플릿의 고정 집계를 제거한다

**파일:**

- 수정: `docs/service-policies/00-policy-template.md`
- 검증만: `docs/service-policies/README.md`

- [ ] **단계 1: 과거 집계 문장을 정본 참조 문장으로 교체한다**

  하드코딩된 `확정 184 / 팀원 상의 필요 18 / 자동 추천 예정 0 / TODO 15` 집계를 포함한 문장을 다음으로 대체한다.
  ...
  예상: 템플릿의 절차 설명은 유지되고 현재 집계 숫자는 README만 소유한다.

- [ ] **단계 2: 템플릿과 README를 검증한다**
  ...
  - 작업 6에서는 README를 수정하지 않는다.

### 작업 7: 비표준 현재 상태값 `개정 확정`을 정규화한다

**파일:**

- 수정: `docs/service-policies/04-reservation.md`의 `RES-003`, `RES-004`, `RES-005` 현재 상태 행
- 수정: `docs/service-policies/README.md`의 `RES-003`, `RES-004`, `RES-005` 마스터 행
- 보존: `docs/service-policies/04-reservation.md`의 결정 기록

- [ ] **단계 1: 정책 본문의 현재 상태 라벨 세 개를 바꾼다**

  `04-reservation.md`에서 다음만 변경한다.
  ...
  - 정책 내용
  - 대체된 역사 행
  - `2026-07-22` 결정 기록 중 결정 상태가 `개정 확정`인 것

- [ ] **단계 2: README 마스터 표의 상태 열 세 개를 바꾼다**

  `RES-003`, `RES-004`, `RES-005`의 상태 열만 `개정 확정`에서 `확정`으로 변경한다.

  예상: 세 정책은 이미 해당 총계에 포함되어 있었으므로 요약 집계는 확정 191개로 유지되고, 기계 판독 가능한 상태 어휘만 일관되게 된다.

- [ ] **단계 3: 상태 라벨과 변경 범위를 검증한다**
  ...
  - 세 마스터 행과 세 정책 제목이 현재 상태 `확정`을 사용한다.
  - `개정 확정`은 역사 결정 기록에만 남아 있다.
  - diff는 예약 규칙, 집계, 중요도 또는 결정 이력을 변경하지 않는다.

### 작업 8: 전체 정책·상태·범위 정합성을 검증한다

**파일:**

- 검증: `docs/service-policies/` 아래의 모든 파일
- 검증: 작업 트리와 변경 파일 allowlist

- [ ] **단계 1: 정책 ID 217개와 상태 집계를 기계적으로 확인한다**
...
- [ ] **단계 4: 비수정 문서를 확인한다**
  ...
  - `README.md`의 변경은 작업 7의 세 상태 셀에만 한정된다.

- [ ] **단계 5: 변경 파일 허용 목록을 확인한다**
  ...
  - 사용자가 별도로 스테이징을 요청하지 않았다면 스테이징된 출력은 비어 있다.

- [ ] **단계 6: 문서 형식과 충돌 문구를 최종 점검한다**

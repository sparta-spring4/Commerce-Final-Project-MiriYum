...
사업자등록번호는 정규화 값을 저장하고 1차 MVP의 일반 등록 경로에서는 폐점 뒤에도 기존 매장이 계속 점유한다. 애플리케이션 형식 검증과 MySQL 유일 제약을 함께 사용해 병렬 등록 중 한 건만 성공하게 한다. 운영 종료는 과거 참조를 연쇄 삭제하지 않으며, 동일 사업자등록 단위의 재개는 OPER-009에 따라 플랫폼 운영자 복구 경로에서 기존 귀속을 다시 검증하는 후속 범위다.

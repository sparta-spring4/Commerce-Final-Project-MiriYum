...
- MySQL 업무 상태와 내구성 사건 레코드를 같은 트랜잭션 또는 동등한 일관성 경계에 기록한다.
- 이벤트에 사건 ID, 대상 유형·ID·버전, 정책 버전을 포함한다.
- 소비자 이름과 사건 ID의 소비 멱등 기록 유일 제약으로 중복 효과를 방지한다.
- 실제 대상 반영과 소비 멱등 기록 후에만 소비 완료 체크포인트를 전진한다.

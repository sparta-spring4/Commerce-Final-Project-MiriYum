...
**범위:** AI 워크플로 문서 구조와 후속 실행 gate 구현 계약

## 1. 목적

MiriYum의 AI 작업이 필요한 문서만 늦게 읽고, 작업 단계에 맞는 검증을 빠뜨리지 않도록 문서 라우팅과 실행 계약을 먼저 정의한다.

이 설계는 프로젝트 인원수를 근거로 기능을 축소하지 않는다. 적용 여부는 작업 복잡도, 변경 위험, 검증 요구, 자동화 가능성과 저장소의 실제 실행 capability로 판단한다.
...
문서 단계에서는 스크립트가 구현될 때 필요한 입력, 출력, 상태, 증거와 실패 의미를 확정한다. 실제 명령, 포트, job 이름처럼 아직 존재하지 않는 runtime 값만 명시적인 미구성 상태로 남긴다.
...
1. 목적과 최소 읽기 원칙
2. Answer Mode, Light Route, Work Route
3. 작업 단계별 문서 확장
4. 주제별 기준 문서 선택
5. 조건부 기준 상태와 runtime 문서
6. 검증 및 완료 단계 확장
7. 문서 변경 라우팅
8. 위임 경로
9. 범위 변경 시 재라우팅 규칙
...
| `ai/context-map.md` | 작업 phase, feature ownership, handoff context 정의 | Work Route 또는 위임이 활성화될 때 |
...
| 파일 | 책임 | 초기 상태 |
|---|---|---|
| `ai/project-state.md` | 알려진 경로, 환경, 포트, helper 상태의 사람용 계약 | `TEMPLATE` / runtime `NOT CONFIGURED` |
| `ai/command-registry.md` | 안정적인 command ID, 선행 조건, 출력, 증거, 실패 의미 | `TEMPLATE` / runtime `NOT CONFIGURED` |
| `ai/cache-policy.md` | 캐시 가능한 정보와 무효화 기준 | `TEMPLATE` |
| `ai/workflow-cache.md` | workflow cache 입력·출력·최신성 계약 | `TEMPLATE` / runtime `NOT CONFIGURED` |
| `ai/native-runtime-adapters.md` | 로컬·CI 실행 adapter 경계와 신뢰 수준 | `TEMPLATE` / runtime `NOT CONFIGURED` |
...
### 3.4 Issue, 서브에이전트와 인계

| 파일 | 책임 | 초기 상태 |
|---|---|---|
| `ai/github-issue-planning.md` | Issue 준비, 범위, 인수 조건과 상태 전환 | `ACTIVE` |
| `ai/github-issue-template.md` | AI가 생성·검토할 Issue 필드 계약 | `ACTIVE` |
| `.github/ISSUE_TEMPLATE/work-item.md` | GitHub에서 사용하는 실제 Issue 입력 surface | `ACTIVE` |
| `ai/subagent-workflow.md` | routing gate, 역할, dispatch, 증거와 재개 규칙 | `ACTIVE` |
| `ai/agent-handoff.md` | 기능 소유, 읽은 문서, 결정, 미해결 질문, 남은 증거 | `ACTIVE` |

Issue가 작업 범위와 담당자를, GitHub Project의 단일 `Status` 필드가 lifecycle을, PR·CI가 실행 결과와 증거를 소유한다. 별도 work log는 같은 상태를 복제하므로 생성하지 않는다.
...
| `ai/verification-gates.md` | gate 진입점, 적용 가능성, 결과 매핑, 증거 경계 | 계약 `ACTIVE`, runtime `NOT CONFIGURED` |
| `ai/qa-gate.md` | 필수 검사, API 증거, 위임 증거, QA 결과 | 계약 `ACTIVE`, runtime 일부 `NOT CONFIGURED` |
| `ai/ci-gates.md` | CI provenance, required check와 내구성 증거 | `TEMPLATE` / runtime `NOT CONFIGURED` |
...
교정 결과는 관련 QA gate와 reviewer checklist로 되돌아가며, 증거 없는 완료는 반려한다.

### 3.6 재사용 가능한 skill 계약

`ai/skills/README.md`는 전체 AI 문서 색인이 아니다. skill 선택이나 위임이 활성화된 경우에만 읽는 skill 카탈로그다.

다음 파일을 모두 만들고 입력, 출력, 선행 조건, 증거, handoff와 실패 상태를 정의한다.

| Skill | 책임 | 초기 runtime 상태 |
|---|---|---|
| `repo-intake.md` | 저장소 진입과 범위 파악 | `CONFIGURED` 가능한 문서 기반 절차 |
| `command-runner.md` | registry 기반 명령 실행 | `NOT CONFIGURED` |
| `verification-runner.md` | 적용 가능한 검증 gate 실행 | `NOT CONFIGURED` |
| `api-smoke-verifier.md` | 실제 API request/response 검증 | `NOT CONFIGURED` |
...
개별 skill 문서는 라우팅 또는 dispatch에서 선택됐을 때만 읽는다. 존재한다는 이유로 일괄 로드하지 않는다.

## 4. 라우팅 동작

### 4.1 Answer Mode

저장소 변경, 규범 결정, 검증 주장 없이 작은 질문에 답하는 경로다. 질문에서 직접 필요한 파일만 읽으며 프로젝트 전체 문서, feature spec, skill catalog와 완료 문서를 기본으로 읽지 않는다.

### 4.2 Light Route

좁은 구조 탐색, 최초 방향 파악, 비규범 문구 또는 색인 유지보수 경로다. 변경이 요구사항, 동작, 계약, 아키텍처, 보안 또는 완료 기준에 영향을 주면 Work Route로 재라우팅한다.
...
1. 실제 backend·frontend 명령 확인과 command registry 활성화
2. command registry machine-readable 표현과 schema
3. 검증 결과 schema와 상태 매핑
4. 로컬 command runner와 verification gate
5. API smoke verifier
6. 실패 triage artifact 계약
7. CI workflow와 required check 연결
8. clean checkout 실행 검증
9. 문서 계약과 실행 결과의 일치 검사

각 runtime surface는 다음 조건을 모두 충족해야 `CONFIGURED`로 전환한다.

- 실제 파일과 명령이 존재한다.
- clean checkout에서 재현된다.
- 성공과 실패 fixture가 모두 검증된다.
- 출력이 문서화된 schema를 만족한다.
- secret, 개인정보와 결제정보가 artifact에 포함되지 않는다.
- CI required check 설정과 실제 workflow 결과가 일치한다.

## 6. 단일 원본과 중복 방지

| 정보 | 단일 원본 |
|---|---|
| 제품 범위와 요구사항 | `docs/01`, `docs/05`, feature spec |
| 상세 비즈니스 정책 | `docs/service-policies/` |
| 아키텍처와 기술 결정 | `docs/06`, ADR |
| 데이터·API 계약 | `docs/07`, feature spec |
| 품질과 운영 기준 | `docs/09` |
| AI 문서 선택 | `ai/document-routing.md` |
| AI phase와 context | `ai/context-map.md` |
| 실행 명령 계약 | `ai/command-registry.md`와 후속 machine registry |
| 작업 범위와 담당자 | GitHub Issue |
| lifecycle 상태 | GitHub Project `Status` |
| 실행 결과와 검증 증거 | PR·CI artifact |

`agent.rules`, guardrail, QA 문서는 이 내용을 복제하지 않고 해당 소유자를 링크하고 적용 시점과 실패 의미만 정의한다.

## 7. 제외 사항

다음은 문서 단계에서 구현하지 않는다.

- 실제 제품 backend·frontend 기능
- 존재하지 않는 Gradle 또는 package 명령의 추측
- 성공 실행이 없는 CI를 active 또는 required로 표시
- 생성된 project state, checksum, cache hit와 runtime attestation의 허위 예시
- Issue·Project·PR 상태를 복제하는 영구 work log
...
- 미구성 runtime 항목에는 활성화 조건, 필수 입력, 예상 출력과 금지 주장이 있다.
- `lazycodex-runbook.md`의 경고 신호가 routing, QA, reviewer와 연결된다.
- skill 7개의 입력, 출력, 증거, handoff와 실패 의미가 정의된다.
...
- runtime 의존 값은 템플릿 메타데이터로 남기되 문서 계약은 완성하도록 한다.
- 문서 재구성 완료 다음 단계에 `AI workflow executable gates` 후속 계획을 추가한다.
- 실행 gate 검증이 끝난 뒤 제품 구현을 시작하도록 순서를 변경한다.

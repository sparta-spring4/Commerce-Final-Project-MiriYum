...
추천 요청 중 외부 지도 호출은 하지 않는다. 거리 기준은 원 매장의 검증된 저장 좌표이고 사용자 현재 위치는 요청·저장·사용하지 않는다. AI/LLM, Spring AI, 벡터 DB, 검색엔진, 추천 전용 서비스·DB·캐시는 `2차 MVP`에 없다.
...
AI 설명·FAQ·RAG, 구독, 채팅, 리뷰, 외부 본인확인과 추가 플랫폼 관리 기능은 기능별 승인 뒤 검토한다. 결정적 코드가 후보·필터·순위를 소유하며, LLM은 승인된 결과 설명만 만들고 실패 시 템플릿으로 대체한다. 이 후보를 근거로 AI 파이프라인, 벡터 DB, 채팅 인프라나 추가 외부 제공자를 미리 설치하지 않는다.

...
방문 예약은 개별 테이블·좌석이 아니라 매장 날짜와 시간 구간별 집계 자원을 사용한다.
...
매장은 다음 세 운영 모드 가운데 승인·업종 조건에 맞는 모드를 선택한다.

| 운영 모드 | 생성하는 거래와 자원 |
|---|---|
| `예약만` | 방문 예약이 날짜·시간 구간별 인원과 팀 수를 점유한다. |
| `예약+메뉴 홀드` | 하나의 방문 거래가 예약 인원·팀 수와 선택 메뉴 수량을 함께 선점하고 전부 확정하거나 전부 반환한다. |
| `픽업 예약` | 카페·베이커리로 승인된 매장만 사용한다. 픽업 제공 구간과 메뉴 수량을 점유하며 방문 예약 인원·팀 수는 만들지 않는다. |
...
- 예약 인원·팀 수 자원: [방문 예약 정책](service-policies/04-reservation.md)

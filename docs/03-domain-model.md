...
- 1차 MVP의 신청 업종, 더미 사업자 기준 데이터의 업종, 검색 태그와 카테고리는 서로 다른 개념이며 자동으로 같은 값으로 취급하지 않는다. 고도화에서 공식 진위확인과 사업자등록증 이미지를 사용하더라도 파일에서 검색 카테고리를 자동 추출하지 않는다.

## 픽업 업종 자격

- `store`는 1차 MVP 더미 사업자 기준 데이터의 업종을 근거로 매장별 픽업 자격 상태를 소유한다.
- 더미 업종이 카페 또는 베이커리로 분류되면 픽업 자격을 부여하고, 그 밖의 값·누락·불일치에는 픽업 자격만 부여하지 않는다. 정확한 코드값은 구현 fixture에서 단일 관리한다.
...
| `픽업 예약` | 1차 MVP 더미 업종이 카페 또는 베이커리로 분류된 매장만 사용한다. 픽업 제공 구간과 메뉴 수량을 점유하며 예약 인원·팀 수는 만들지 않는다. |

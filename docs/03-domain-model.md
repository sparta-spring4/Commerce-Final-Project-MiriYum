...
- `store`는 1차 MVP 등록 신청의 필수 업종 `CAFE`, `BAKERY`, `OTHER`를 근거로 매장별 픽업 자격 상태를 소유한다.
- 등록 업종이 `CAFE` 또는 `BAKERY`이면 픽업 자격을 부여하고 `OTHER`에는 픽업 자격만 부여하지 않는다.
...
- `pickup`은 픽업 활성화와 확정 시 중앙 자격 상태를 소비할 뿐 검색 카테고리·태그에서 자격을 추론하거나 생성하지 않는다.
...
다음 세 항목은 상호 배타적인 매장 유형이 아니라 거래별 운영 모드·활성 기능이다. 매장은 입점·기능 조건에 맞는 항목을 함께 활성화할 수 있고, 등록 업종이 `CAFE` 또는 `BAKERY`여서 중앙 픽업 자격을 얻은 정상 매장은 `1차 MVP`부터 `픽업 예약`을 `예약만` 또는 `예약+메뉴 홀드`와 병행할 수 있다.

| 운영 모드 | 생성하는 거래와 자원 |
|---|---|
| `예약만` | 예약이 날짜·시간 구간별 인원과 팀 수를 점유한다. |
| `예약+메뉴 홀드` | 하나의 예약 거래가 예약 인원·팀 수와 선택 메뉴 수량을 함께 선점하고 전부 확정하거나 전부 반환한다. |
| `픽업 예약` | 1차 MVP 등록 업종이 `CAFE` 또는 `BAKERY`인 매장만 사용한다. 픽업 제공 구간과 메뉴 수량을 점유하며 예약 인원·팀 수는 만들지 않는다. |

...
Flyway 기준 전체 물리 테이블·FK·논리 참조는 [전체 물리 ERD](erd/README.md)를 기준으로 확인한다. 이 문서는 제품 관계와 핵심 논리 관계만 다룬다.
...
- 1차 MVP의 등록 업종과 검색 태그·카테고리는 서로 다른 개념이며 자동으로 같은 값으로 취급하지 않는다. 고도화에서 공식 진위확인과 사업자등록증 이미지를 사용하더라도 파일에서 검색 카테고리를 자동 추출하지 않는다.

## 픽업 업종 자격

- `store`는 1차 MVP 등록 신청의 필수 업종 `CAFE`, `BAKERY`, `OTHER`를 등록·분류 정보로 소유하며 픽업 가능 여부에는 사용하지 않는다.
- 모든 등록 업종은 `pickupEnabled`로 픽업 예약 기능을 선택할 수 있다.
- `businessType`은 1차 MVP 신청자의 자기신고 값이므로 실제 업종과 다른 제출을 기술적으로 차단하지 않는다. 국세청 공식 진위확인 응답과 버전된 허용 매핑의 도입·기존 자격 전환은 고도화 진입 전 별도 계약으로 확정하며 1차 MVP 구현 의존성으로 넣지 않는다.
- `pickup`은 픽업 확정 시 Store 공개 계약으로 승인·영업·`pickupEnabled` 상태를 검증하고 등록 업종·검색 카테고리·태그를 판정에 사용하지 않는다.
...
다음 세 항목은 상호 배타적인 매장 유형이 아니라 거래별 운영 모드·활성 기능이다. 정상 매장은 등록 업종과 무관하게 `1차 MVP`부터 `픽업 예약`을 `예약만` 또는 `예약+메뉴 홀드`와 병행할 수 있다.

| 운영 모드 | 생성하는 거래와 자원 |
|---|---|
| `예약만` | 예약이 날짜·시간 구간별 인원과 팀 수를 점유한다. |
| `예약+메뉴 홀드` | 하나의 예약 거래가 예약 인원·팀 수와 선택 메뉴 수량을 함께 선점하고 전부 확정하거나 전부 반환한다. |
| `픽업 예약` | 등록 업종과 무관하게 `pickupEnabled`인 매장이 사용한다. 픽업 제공 구간과 메뉴 수량을 점유하며 예약 인원·팀 수는 만들지 않는다. |

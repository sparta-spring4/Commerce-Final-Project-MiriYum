...
4. 교체 뒤 service의 `desiredStatus=STOPPED` task 전체를 다시 열거하고 직접 조회한다. 사전 snapshot 이후 생긴 task를 포함해 누락·lookup failure가 없고 모두 `lastStatus=STOPPED`여야 한다. `RUNNING`, `DEACTIVATING`, `STOPPING` 등 비종결 task가 하나라도 남아 있으면 실패한다.
5. 교체 뒤 service의 `desiredStatus=RUNNING` task를 다시 조회하고 전부 새 task definition의 `RUNNING` task이며 수가 desired count와 같은지 확인한다. 각 task의 `ATTACHED` ENI에서 단일 `privateIPv4Address`도 함께 확보한다.
6. service가 사용하는 모든 target group을 조회하고 target 수가 service desired count와 같으며 전부 `healthy`여야 한다. 또한 각 target group의 healthy `Target.Id` 집합은 현재 task의 private IPv4 집합과 정확히 같아야 한다. 새 task 미등록, stale/manual target 혼입, `draining`, `unused`, `unhealthy` 또는 누락된 target health는 deregistration 미완료로 판정한다.

...
The staging Compose file fixes `MIRIYUM_RUNTIME_ENVIRONMENT=staging`; operators cannot change that marker through the server-local `.env`. `MIRIYUM_STAGING_LOAD_TEST_SOURCE_IP` is empty by default, and the backend permits an exception only for one valid public IP and only for login and token-refresh rate-limit categories. Production does not receive the staging runtime marker, so setting only the source-IP variable there does not enable the exception. CIDR, multiple values, hostnames, and non-public addresses are rejected when staging starts.

Use the exception only during the approved #357 window:

1. Confirm the approved full deployment SHA, execution time, load caps, synthetic fixture, and the runner's current public IP. Do not put the IP in an Issue, PR, chat transcript, shell command, or retained artifact.
...
4. Run the approved smoke before the baseline. Store only execution time, success/429/5xx counts, p50/p95/p99, scenario inputs, and the deployed full SHA. Never retain the source IP, tokens, cookies, authorization headers, or raw HTTP output.

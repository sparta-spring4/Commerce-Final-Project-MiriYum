...
위험 marker는 최초 생성 시점부터 7일 뒤 자동 만료된다. 같은 marker 재사용은 발생 횟수만 누적하고 만료 시각을 연장하지 않는다. 1시간 이상 전달되지 않은 marker가 있으면 backend가 식별자 없이 다음 로그를 남기고, CloudWatch는 `$long_stay_count`를 숫자 지표로 추출한다.
...
정상 결과에는 `"$long_stay_count": "2"`가 포함된다. 이 검증 화면은 로그 접두어와 숫자 추출 규칙을 함께 보여 주므로 블로그 증빙으로 캡처할 수 있다. AWS 계정 ID, SNS Topic ARN, 이메일 주소는 가린다.

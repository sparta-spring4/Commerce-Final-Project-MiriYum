...
9. The prepared workflow schedules the fixed interruption and posts `SSE_RECOVERY_ARMED run_id=... rendezvous_id=... execute_at_epoch=...` as `github-actions[bot]` only after SSM is `InProgress`. k6 accepts only that exact bounded marker and performs the approved mutation at the reviewed offset inside the ten-second interruption.

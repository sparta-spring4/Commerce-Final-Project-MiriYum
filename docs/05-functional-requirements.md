...
정책 결정 상태 집계와 현재 MVP 미결정 세부 집계도 분리한다.

- 정책 ID 상태는 `확정` 195개, `팀원 상의 필요` 0개, `TODO` 22개로 합계 217개다.
- 현재 MVP에서 팀이 결정해야 할 세부는 `AUTH-007`의 토큰 수명 2건, 브라우저 전달·저장과 CSRF 경계, 동시 로그인 수, 비밀번호·이메일 변경 시 폐기 범위, 만료 직전 민감 작업 재검증 범위의 6건이다.
- `TASTE-003`, `TASTE-008`, `TASTE-012`, `SUB-001`, `SUB-002`, `SUB-003`, `SUB-007`, `ADS-001`은 `TODO + 비초기`이므로 현재 MVP 미결정 집계에 포함하지 않는다.
...
| 회원·인증·계정 | AUTH-001, AUTH-002, AUTH-003, AUTH-004, AUTH-005, AUTH-006, AUTH-007, AUTH-008, AUTH-009, AUTH-010, AUTH-011, AUTH-012 | 부분 초기 | auth | [정책 원본](service-policies/01-member-auth.md) | 현재 계정·인증 흐름과 Spring Security 기반 액세스 JWT·리프레시 토큰·Valkey 리프레시 상태 관리는 초기 범위다. `AUTH-004`의 연령 확인 방향은 확정이지만 주류 기능과 남은 세부는 현재 MVP에서 제외한다. `AUTH-007` 구조는 확정이지만 토큰 수명·브라우저 경계 등 세부 6건은 현재 MVP의 팀원 상의 필요 항목이다. |
...
| 메뉴 홀드 | HOLD-001, HOLD-002, HOLD-003, HOLD-004, HOLD-005, HOLD-006, HOLD-007, HOLD-008, HOLD-009, HOLD-010, HOLD-011, HOLD-012, HOLD-013 | 초기 핵심 | booking | [정책 원본](service-policies/06-menu-hold.md) | `booking`은 예약 연계 메뉴 홀드와, 홀 운영 여부와 관계없이 카페·베이커리에만 허용하는 단독 픽업 홀드를 제공한다. 일반 식당과 다른 업종에는 단독 픽업 홀드를 제공하지 않으며 상세 정책의 수량·복구 기준을 함께 적용한다. |
| 코스·테이스팅 | TASTE-001, TASTE-002, TASTE-003, TASTE-004, TASTE-005, TASTE-006, TASTE-007, TASTE-008, TASTE-009, TASTE-010, TASTE-011, TASTE-012 | 비초기 | booking | [정책 원본](service-policies/07-course-tasting.md) | 코스·테이스팅, 관련 선결제·설문·행사·회차 기능은 1차 MVP에서 제외한다. 미결정인 `TASTE-003`, `TASTE-008`, `TASTE-012`는 `TODO + 비초기`다. |
| 결제·환불·정산 | PAY-001, PAY-002, PAY-003, PAY-004, PAY-005, PAY-006, PAY-007, PAY-008, PAY-009, PAY-010, PAY-011, PAY-012, PAY-013, PAY-014, PAY-015 | 부분 초기 | payment | [정책 원본](service-policies/08-payment-refund.md) | 예약금·취소·환불은 PortOne V2를 사용하며, 실제 활성 PG 채널·결제수단은 계약·심사·운영 구성 완료 여부를 따른다. 결제 그룹의 명시적 TODO는 실제 식당 정산 `PAY-012`뿐이다. 메뉴 가격 선결제와 비초기 결제 기능은 포함하지 않는다. |
...
| 취소 자리·수량 자동 승계 | TRANSFER-001, TRANSFER-002, TRANSFER-003, TRANSFER-004, TRANSFER-005, TRANSFER-006, TRANSFER-007, TRANSFER-008, TRANSFER-009 | 초기 핵심 | booking | [정책 원본](service-policies/10-waitlist-transfer.md) | 확정 예약의 유효한 일반 예약 취소로 반환된 수용량과 `HOLD-010`에 따른 메뉴 취소 수량의 대기 등록·FIFO 제안·선점·전환 흐름을 제공한다. 변경 감소·거절·만료·시간 경과·노쇼는 예약 자동 승계 원인이 아니며 노쇼는 일반 가용 재공개도 만들지 않는다. |
| 사용자·식당 구독 | SUB-001, SUB-002, SUB-003, SUB-004, SUB-005, SUB-006, SUB-007, SUB-008, SUB-009, SUB-010 | 비초기 | payment | [정책 원본](service-policies/11-subscription.md) | 사용자 구독과 매장 Pro, 구독자 전용 혜택·유료 청구는 1차 MVP에서 제외한다. 미결정인 `SUB-001`, `SUB-002`, `SUB-003`, `SUB-007`은 `TODO + 비초기`다. |
| 리뷰·신뢰·어뷰징 | TRUST-001, TRUST-002, TRUST-003, TRUST-004, TRUST-005, TRUST-006, TRUST-007, TRUST-008, TRUST-009, TRUST-010, TRUST-011, TRUST-012 | 비초기 | review | [정책 원본](service-policies/12-review-trust.md) | 리뷰 작성·공개·수정·삭제, 별점·태그, 사진·텍스트, 신고·검수, 식당 답글, 조작 탐지, 신뢰 점수·제재·오탐 복구는 1차 MVP에서 제외한다. 확정 정책은 향후 도입 기준만 보존한다. |
| 광고·추천 | ADS-001, ADS-002, ADS-003, ADS-004, ADS-005, ADS-006, ADS-007, ADS-008 | 부분 초기 | store | [정책 원본](service-policies/13-ad-recommendation.md) | 하나의 검색창에서 검색어와 구조화 조건을 함께 추출하는 통합 검색, 규칙 우선·선택적 외부 AI 보조·MySQL 직접 조회와 취향·품절 대체 추천은 초기 무료 기능이다. OpenSearch와 Meilisearch는 도입 여부가 확정된 MVP 제외 기술이며 `ADS-001` 광고 상품은 `TODO + 비초기`다. 광고 과금·광고 제거도 제외한다. |
| 분석·수요 리포트 | ANALYTICS-001, ANALYTICS-002, ANALYTICS-003, ANALYTICS-004, ANALYTICS-005, ANALYTICS-006, ANALYTICS-007, ANALYTICS-008, ANALYTICS-009 | 부분 초기 | store | [정책 원본](service-policies/14-analytics-report.md) | 매장 Free 기본 운영 통계는 초기 범위다. Pro 비교·해석·추천·자동 리포트·내보내기는 제외한다. |

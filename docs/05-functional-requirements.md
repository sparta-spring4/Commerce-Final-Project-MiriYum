...
- `AUTH-007`의 토큰 수명 2건, 브라우저 전달·저장과 CSRF 경계, 동시 로그인 수, 비밀번호·이메일 변경 시 폐기 범위, 만료 직전 민감 작업 재검증 범위의 6건은 팀 합의 전 임의로 고정하지 않는다.
...
| 취소 자리·수량 자동 승계 | TRANSFER-001, TRANSFER-002, TRANSFER-003, TRANSFER-004, TRANSFER-005, TRANSFER-006, TRANSFER-007, TRANSFER-008, TRANSFER-009 | 확정 | `고도화` | booking | [정책 원본](service-policies/10-waitlist-transfer.md) | 확정 예약의 유효한 일반 취소와 메뉴 홀드 취소 복구 완료분만 대상이다. 변경 감소·거절·만료·시간 경과·노쇼는 원인이 아니다. |
...
| 광고·추천 | ADS-001, ADS-002, ADS-003, ADS-004, ADS-005, ADS-006, ADS-007, ADS-008 | 확정·TODO 혼재 | `2차 MVP`, `향후 고도화` | store | [정책 원본](service-policies/13-ad-recommendation.md) | 2차는 `RuleInterpreter`, QueryDSL, 결정적 Java 점수화·이력 추천·품절 대안·지도 탐색이다. 향후 LLM은 코드가 정한 TOP3 설명만 생성하고 실패 시 템플릿을 사용한다. 광고 상품도 향후 경계다. |

...
관리자 전용 snapshot이나 projection 테이블은 두지 않는다. V62의 MenuHold `status_version`과 append-only 전이 원장은 원 도메인 이력 보강이며, 기존 행의 baseline 이전 시점은 복원 불가 경계로 유지한다. 목록에는 민감 주체 정보를 포함하지 않고 상세도 내부 사용자·행위자·감사 사유 원문 없이 최소 상관 ID만 제공한다. HTTP 권한·부분 실패·cursor 계약은 [예약·웨이팅 통합 모니터링 명세](specs/admin-monitoring/spec.md)를 따른다.

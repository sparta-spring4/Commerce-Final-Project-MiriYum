...
| Flow | 주요 참여자 | 핵심 상태·자원 | 기본 다이어그램 |
|---|---|---|---|
| 전체 사용자 여정 | 사용자, 매장 | 예약, 메뉴 수량, 예약금, 방문 상태 | Flowchart |
| 통합 예약 | 사용자, 매장, 결제사 | 수용량 선점, 메뉴 선점, 결제 | Flowchart |
| 현장·원격 웨이팅 | 사용자, 매장 | 대기 순번, 호출, 도착 | Flowchart |
| 예약·웨이팅 전환 | 사용자, 매장 | 기존 거래, 전환 선점 | Flowchart |
| 취소 자원 자동 승계 | 사용자, 매장 | 반환 자원, 승계 제안 | Flowchart |
| QR 체크인·노쇼 | 사용자, 매장 | 체크인, 방문 완료, 노쇼 | Flowchart |
...
5. 예약과 메뉴 홀드를 함께 확정하고, 방문 시 현재 이용 자격에 따라 회전형 QR을 조회·발급한다.
...
flowchart TD
    ...
    F --> F1{"결제 처리 결과"}
    F1 -- "성공" --> G
    F1 -- "명확한 실패" --> F2["선점 해제·예약 미확정"]
    F1 -- "결과 확인 필요" --> F3["선점 보호·결제 결과 확인"]

    C -- "웨이팅" --> H["위치·중복 확인"]
    H --> I["FIFO 대기열 등록"]
    I --> J["매장 입장 호출"]

    G --> K["방문 시 회전형 QR 조회·발급"]
    J --> L["매장 방문"]
    K --> L

    L --> M{"방문 확인?"}
    M -- "예" --> N["체크인·방문 완료"]
    N --> O["예약금이 있는 예약이면 환불 정책 적용"]
    M -- "아니오" --> P["노쇼 확인·처리"]
```
...
- 동일 사용자의 중복 예약과 같은 매장·겹치는 영업 구간의 활성 예약·웨이팅 존재 여부를 확인해야 한다.
...
flowchart TD
    ...
    J -- "성공" --> H
    J -- "명확한 실패" --> K["선점 해제 및 실패 안내"]
    J -- "결과 확인 필요" --> N["선점 격리·PG 결과 확인"]
...
| 대상 | 상태 변화 |
|---|---|
| 정상 웨이팅 | `대기 중` → `호출 중` → `도착 확인` → `체크인 완료` |
| 미응답 | `대기 중` → `호출 중` → `호출 실패` → `미응답 종료` |
| 사용자 취소 | `대기 중` → `사용자 취소` |
| 매장 종료 | `대기 중` → `매장 종료` |
...
같은 사용자가 같은 매장·겹치는 영업 구간에 활성 예약 또는 웨이팅을 보유한 상태에서 반대 거래를 요청하면, 기존 거래를 먼저 잃지 않고 취소·환불 조건과 예약금 필요 여부를 확인한 뒤 안전하게 전환할 수 있게 한다.
...
- 사용자가 같은 매장·겹치는 영업 구간에 활성 예약 또는 활성 웨이팅을 보유하고 있어야 한다.
- 기존 거래와 전환 대상 거래의 상태, 사용자 자격 및 대상 자원의 가용성을 다시 검증해야 한다.
- 예약 취소가 필요한 경우 `RES-009`·`PAY-009`가 계산한 취소 수수료·환불 결과와 적용 정책 버전을 먼저 표시하고 사용자의 명시적 확인을 받아야 한다.
- 웨이팅에서 예약으로 전환하면서 예약금이 필요한 경우에는 검증된 결제 성공 후에만 예약을 확정해야 한다.

### 주요 흐름

- 기존 거래가 발견되면 시스템은 신규 요청을 즉시 확정하지 않고 기존 거래 유지 또는 조건부 전환 선택지를 표시한다.
- 예약에서 웨이팅으로 전환할 때는 웨이팅 접수 가능성·운영 상태·영업 구간·중복 등록 여부를 먼저 확인한다. 조건을 충족하면 예약 종결, 웨이팅 생성 및 필요한 환불 작업 생성을 하나의 전환 사건으로 확정한다.
- 웨이팅에서 예약으로 전환할 때는 예약 수용량을 10분간 선점하고, 기존 대기 팀을 비종결 `예약 전환 중`으로 변경한다.
- `예약 전환 중`에는 원 대기 팀 ID·등록 시각·queue sequence를 보존하되 해당 팀을 호출 대상에서 제외한다. 화면에 표시되는 숫자 순번은 현재 활성 대기열을 기준으로 다시 계산될 수 있다.
- 예약금이 없거나 결제 불필요 판정이 검증되면 예약 확정과 웨이팅의 `예약 전환 완료` 종결을 하나의 전환 결과로 확정한다.
- 예약금이 필요한 경우에는 검증된 결제 성공 후 예약을 확정하고 웨이팅을 `예약 전환 완료`로 종결한다.

### 예외 흐름

- 웨이팅 등록 조건 또는 예약 수용량을 확보할 수 없으면 새 거래를 생성하지 않고 기존 거래를 유지한다.
- 사용자가 웨이팅→예약 전환을 철회하거나 결제 실패 또는 10분 선점 만료가 확정되면 예약 선점을 해제하고, 기존 팀 ID·등록 시각·queue sequence를 유지한 채 `대기 중`으로 복귀시킨다.
- `대기 중`으로 복귀한 팀의 화면상 숫자 순번은 현재 활성 대기열을 기준으로 다시 계산한다. 전환 실패를 이유로 새 queue sequence를 발급하거나 대기열 마지막으로 보내지 않는다.
- 결제 결과를 즉시 확정할 수 없으면 `예약 전환 중` 상태와 예약 수용량 선점을 함께 보호한다. 확인이 끝나기 전에는 예약 확정, 선점 해제·재판매, 웨이팅 복귀 또는 웨이팅 종결을 추정하여 처리하지 않는다.
- 전환 실패 시점에 해당 영업 구간의 매장 종료가 이미 확정됐다면 웨이팅으로 복귀시키지 않고 제재 없는 `매장 종료` 상태로 종결한다.

### Mermaid 플로우차트

```mermaid
flowchart TD
    A{"같은 매장·겹치는 영업 구간에 기존 거래가 있는가?"}
    A -- "없음" --> B["신규 예약·웨이팅 진행"]
    A -- "있음" --> C["기존 거래 유지 또는 조건부 전환 안내"]
    C --> D["취소 수수료·환불 결과·예약금 조건 안내"]
    D --> E{"사용자가 전환을 확인했는가?"}

    E -- "아니오" --> F["기존 거래 유지"]
    E -- "예" --> G{"전환 방향"}

    G -- "예약 → 웨이팅" --> H["웨이팅 접수·운영 상태·중복 조건 확인"]
    H --> I{"웨이팅 생성 가능한가?"}
    I -- "아니오" --> F
    I -- "예" --> J["예약 종결·웨이팅 생성·환불 작업 기록"]

    G -- "웨이팅 → 예약" --> K["예약 수용량 10분 선점"]
    K --> L["기존 팀 ID·등록 시각·queue sequence 보존"]
    L --> M["호출 대상 제외·예약 전환 중으로 변경"]
    M --> N{"예약금 처리 결과"}

    N -- "성공 또는 결제 불필요" --> O["예약 확정·웨이팅 전환 완료"]
    N -- "명확한 실패·선점 만료·철회" --> P["예약 선점 해제"]
    P --> Q{"매장 운영 중인가?"}
    Q -- "예" --> R["기존 팀·queue sequence로 대기 중 복귀"]
    Q -- "아니오" --> S["제재 없이 매장 종료"]

    N -- "결과 확인 필요" --> T["예약 전환 중·수용량 선점 보호"]
    T --> U["PG 원본·중앙 전환 원장 확인"]
    U --> N
```
...
| 대상 | 상태 변화 |
|---|---|
| 내부 체크인 처리 | `체크인 요청` → `검증` → `체크인 완료` |
| 사용자 공개 예약 상태 | `예약 완료` → `방문 완료` |
| 직접 노쇼 처리 | `예약 완료` → 매장 권한·상태·사유 확인 → `노쇼` |
| 장애·귀책 불명확 | 기존 `예약 완료` 유지, 사용자 귀책·노쇼 미확정 |

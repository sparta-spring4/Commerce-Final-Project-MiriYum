...
- QR·일회 확인번호·직원 확인을 통한 체크인과 노쇼 처리
...
- QR 체크인은 1차 MVP에 포함하며 일회 확인번호와 직원 확인을 보조 경로로 제공한다.
...
5. 예약과 메뉴 홀드를 함께 확정하고 QR 또는 일회 확인번호를 발급한다.
...
- 결제 실패 또는 선점 만료 시 기존 queue sequence로 웨이팅에 복귀한다.
...
- 서버가 회전형 QR 또는 짧은 일회 확인번호를 발급해야 한다.
- 매장 운영자는 자신이 관리하는 매장의 현재 이행 대상만 확인할 수 있다.
- 취소, 만료, 이미 완료된 거래는 체크인할 수 없다.

### 주요 흐름

1. 사용자가 QR 또는 일회 확인번호를 매장에 제시한다.
2. 매장이 QR을 스캔하거나 확인번호를 입력한다.
...
- 매장 확인이 없으면 6시간 후 노쇼 후보로 전환하고, 24시간 안에 정정·이의가 없을 때 자동 확정한다.
- 체크인 또는 알림 장애처럼 사용자 귀책이 불명확한 경우 자동 노쇼 확정을 중단한다.
...
| 노쇼 | `방문 예정` → `노쇼 후보` → `노쇼 확정` |
| 이의·장애 | `노쇼 후보` → `검토` → `방문 완료` 또는 `노쇼 확정` |

### Mermaid 플로우차트

```mermaid
flowchart TD
    A["예약 방문 시각 도달"] --> B["QR·일회 확인번호 제시"]
    B --> C{"체크인 방식"}
...
- QR·확인번호·직원 확인은 같은 중앙 거래를 기준으로 정확히 한 번 방문 완료 처리한다.
...
| 노쇼 | 사용자 귀책 불명확·이의 접수 | 자동 확정 중단 및 검토 |
...
- QR 토큰의 구체적인 회전 주기와 일회 확인번호 길이·만료시간은 구현 명세에서 확정해야 한다.

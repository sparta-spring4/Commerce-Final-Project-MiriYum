...
- 운영 delivery path는 Terraform state로 reconciliation하고, 비용 절감 휴지에는
  ECS service, ALB, listeners, target groups, NAT gateway/EIP와 `api` alias를
  제거한 뒤 필요 시 다시 생성한다.
- RDS 데이터 저장소, VPC 기반 네트워크, security group, hosted zone, secret과
  immutable ECR 이미지는 lifecycle destroy 대상이 아니다. RDS는 별도 stop/start
  절차로만 중지·재개한다.
- ECS는 blue/green target group과 production/test listener rule을 사용한다.
  배포 controller의 임시 listener weight는 Terraform drift가 아니며, topology
  자체는 Terraform이 소유한다.

### 검증·복구 경계

- 시작 전 Terraform plan은 retained data/network resource의 destroy가 없는지
  검토해야 한다. 계획에 예기치 않은 destroy가 있으면 apply하지 않는다.
- 시작 뒤 ECS `ACTIVE`, `runningCount = desiredCount`, `pendingCount = 0`, active
  target group healthy, public actuator health `UP`을 모두 확인한다.
- application rollback은 호환성을 확인한 직전 task definition 배포로 한정한다.
  RDS restore 또는 Terraform destroy는 application rollback 수단이 아니다.

### 관련 런북

- [Production Terraform lifecycle](../deployment/production-terraform-lifecycle.md)
- [Production ECS incident runbook](../deployment/production-ecs-incident-runbook.md)

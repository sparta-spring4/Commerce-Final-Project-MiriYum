...
`Auto`는 Docker Engine 또는 Windows Docker Desktop의 WSL2 VM에 설정된 별도 메모리 상한과 그 내부 가용량을 조회하지 않는다. 위 임계값은 Docker가 OS가 보고한 호스트 가용 RAM 안에서 선택한 integration worker 수만큼 Testcontainers MySQL을 실행할 용량을 제공한다는 가정에 한정된다. `.wslconfig`나 Docker Desktop 설정으로 호스트와 별도의 더 작은 상한을 둔 환경은 `-ParallelShards 1|2|4`로 상한에 맞는 값을 명시해야 한다. runner는 Auto 선택 근거를 출력할 때 이 Docker 메모리 비관측 경계와 명시적 override 안내를 함께 경고하며, Auto 선택값을 Docker VM 안전성 보장으로 표현하지 않는다.
...
Ubuntu process 실행은 `System.Diagnostics.ProcessStartInfo`를 사용하고 Wrapper 실행 파일과 각 인자를 `ArgumentList`로 전달한다. Windows는 `ProcessStartInfo`를 사용하지 않고 `CreateProcessW`와 `CREATE_SUSPENDED`를 사용하는 native launcher가 process 생성을 전담한다. native launcher는 stdin·stdout·stderr용 anonymous pipe를 만들고 child-side stdin read handle과 stdout·stderr write handle만 상속 가능하게 생성한다. parent-side stdin write handle과 stdout·stderr read handle은 비상속으로 설정한다. `STARTUPINFOEX.StartupInfo.cb`는 `sizeof(STARTUPINFOEX)`로 설정하고, `StartupInfo.dwFlags`에 `STARTF_USESTDHANDLES`를 지정하며, `hStdInput`, `hStdOutput`, `hStdError`에 각각 유효한 child-side stdin read, stdout write, stderr write handle을 설정한다. `InitializeProcThreadAttributeList`의 첫 호출로 필요한 크기를 구해 buffer를 할당하고 두 번째 호출로 attribute list를 초기화한 뒤 `UpdateProcThreadAttribute`로 정확히 세 child-side standard handle만 `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`에 등록한다. attribute list 초기화가 성공한 뒤에는 `UpdateProcThreadAttribute` 또는 `CreateProcessW`의 호출·성공 여부와 관계없이 모든 경로에서 `DeleteProcThreadAttributeList`를 호출하고 buffer도 회수한다. `CreateProcessW`는 `bInheritHandles=TRUE`와 `EXTENDED_STARTUPINFO_PRESENT`를 함께 사용한다. `CreateProcessW` 성공 직후 parent가 보유한 세 child-side handle 복사본을 닫는다. 일반 runner는 parent-side stdin write handle도 즉시 닫아 child에 EOF를 제공하고, stdout·stderr parent read handle의 비동기 pump를 시작한 뒤 Job 귀속과 resume을 수행한다. contract fixture는 nonce를 parent-side stdin write handle에 기록한 뒤 닫고 child가 읽은 동일 Unicode 값을 stdout으로 반환하는지 확인해 stdin→child→stdout 연결을 검증한다. 별도의 inheritable sentinel handle 하나를 의도적으로 만들되 handle allowlist에서는 제외하고, child가 전달받은 해당 handle 값으로 접근을 시도했을 때 사용할 수 없음을 확인해 allowlist가 실제로 적용됐음을 음성 검증한다. `ResumeThread` 성공 직후 thread handle을 닫고, process handle은 process 종료 대기와 `GetExitCodeProcess`가 끝날 때까지, parent read handle은 각 pump가 EOF를 받고 완료될 때까지, Job handle은 `ActiveProcesses == 0` 확인이 끝날 때까지 유지한 뒤 닫는다. 생성·귀속·resume·pump 중간 실패를 포함한 모든 경로가 동일한 소유권 순서로 handle을 회수하며, parent의 child-side write handle 또는 stdin write handle 잔존으로 EOF를 막아서는 안 된다. `cmd.exe /d /s /c` 뒤 단일 mutable command-line buffer를 구성하는 adapter는 공백·한글·`&`, `(`, `)`, `^`, `%`, `!`와 따옴표(`"`)가 포함된 경로·인자를 보존해야 한다. contract fixture는 따옴표 단독 입력과 따옴표가 공백·한글·각 meta character와 결합된 입력을 각각 child에 전달하고, child가 실제 수신한 argv가 원본과 byte-for-byte가 아니라 Unicode 문자열 값 기준으로 정확히 일치하는지 검증한다.
...
- process cleanup 성공을 Docker container 0건으로 보고하지 않고 Testcontainers container 회수는 Ryuk에 위임하며 run-id label 기반 직접 추적·강제 정리는 제외하는 경계
- Windows native launcher가 `STARTUPINFOEX.StartupInfo.cb = sizeof(STARTUPINFOEX)`, `STARTF_USESTDHANDLES`, 유효한 `hStdInput`·`hStdOutput`·`hStdError`, `bInheritHandles=TRUE`를 함께 적용하고, `InitializeProcThreadAttributeList` 크기 조회·할당·초기화 → `UpdateProcThreadAttribute` → `DeleteProcThreadAttributeList`·buffer 회수 수명 주기로 정확히 세 child-side standard handle만 `PROC_THREAD_ATTRIBUTE_HANDLE_LIST`에 등록하는 계약
- Windows fixture가 allowlist 밖에 별도 inheritable sentinel handle을 만들고 child가 해당 handle을 사용할 수 없음을 확인해 비허용 handle 비상속을 음성 검증하는 계약

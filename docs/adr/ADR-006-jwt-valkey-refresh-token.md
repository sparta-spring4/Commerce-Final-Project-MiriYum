...
- 절대 상한 뒤에 별도 tombstone 또는 전용 종료 상태를 보관하지 않는다. 상한에 맞춰 Refresh JWT와 Valkey family TTL이 함께 만료되면 Refresh JWT 검증이 먼저 기존 `AUTH_008`로 종료되므로, 정상적인 상한 만료·후속 재시도는 재사용 위험 사건을 만들지 않는다.
- 새 만료 시각이 최초 로그인 + 30일에 의해 잘리는 회전은 민감 식별자 없는 구조화 로그와 지표로 관측한다. 이 신호는 절대 상한 정책이 실제로 적용됐는지 확인하기 위한 것이며, 만료된 JWT를 다시 파싱하거나 상태를 되살려 구분하지 않는다.

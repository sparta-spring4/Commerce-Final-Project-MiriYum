...
- 전달 작업은 `SSCAN`으로 pending Set을 읽고 한 주기에 marker 100개까지만 처리한다. marker가 비어 보이면 Lua에서 marker 부재 확인과 `SREM`을 함께 수행하므로, 같은 키의 marker가 전달 중 다시 생성되어도 새 인덱스 연결을 삭제하지 않는다.

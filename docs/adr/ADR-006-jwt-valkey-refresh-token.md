...
- **상태: 제안.** 팀 검토로 확정되기 전까지 프로덕션 코드의 만료 계산은 변경하지 않는다.
...
- 절대 상한에 도달하면 새 Refresh 상태를 만들지 않고 기존 `AUTH_008`로 재로그인을 요구한다. 별도 오류 코드나 응답 세부 정보는 추가하지 않아, 클라이언트는 일반 Refresh Token 만료와 동일하게 로그인 화면으로 이동한다.
- 후속 구현은 절대 상한 도달을 `NOT_FOUND`, `REUSED`와 구분되는 내부 회전 결과(예: `ABSOLUTE_EXPIRED`)로 처리한다. 외부 응답은 계속 `AUTH_008`로 통일하되, 서버는 민감 식별자 없는 구조화 로그와 지표로 상한 도달 횟수를 관측한다.
- 절대 상한에 도달한 현재 family는 원자적으로 절대 상한 전용 종료 상태로 전환한다. 같은 토큰의 후속 재시도는 이 상태를 다시 반환하며 `REUSED_ROTATED_TOKEN`·`REUSED_REVOKED_TOKEN` 위험 사건을 만들지 않는다. 교체된 이전 토큰처럼 실제 재사용 조건을 만족하는 요청만 기존 위험 사건 정책을 적용한다.
- 배포 전에 생성되어 `familyCreatedAt`이 없는 legacy family는 backfill하지 않는다. 첫 회전에도 기존 family의 남은 만료 시각을 유지하고 만료를 다시 연장하지 않는다. 따라서 배포 시점에 남은 Refresh Token 최대 수명인 14일 동안만 기존 sliding 정책이 제한적으로 공존하며, 이후 새 로그인으로 생성된 family에만 30일 절대 상한이 적용된다.

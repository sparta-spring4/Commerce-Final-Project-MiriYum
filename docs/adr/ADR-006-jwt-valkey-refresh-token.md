...
토큰 유효기간은 [AUTH-007](../service-policies/01-member-auth.md#auth-007-세션토큰기기-관리)을 정본으로 한다. 이 PR은 Access JWT 15분과 Refresh JWT 최대 14일을 유지하면서, 최초 로그인부터 최대 30일의 절대 세션 만료를 두는 방안을 제안한다. Refresh Token 회전이 성공하면 새 발급 시각을 기준으로 최대 14일을 부여하는 sliding 만료를 적용한다. 단, family는 최초 로그인 시각부터 최대 30일의 절대 세션 만료를 넘길 수 없다. 새 Refresh JWT와 Valkey family TTL은 `min(회전 시각 + 14일, 최초 로그인 시각 + 30일)`로 함께 갱신하며, 상한에 도달한 family는 재발급 대신 새 로그인을 요구한다. 팀 검토로 수치가 확정되기 전까지 프로덕션 코드의 만료 계산은 변경하지 않는다.

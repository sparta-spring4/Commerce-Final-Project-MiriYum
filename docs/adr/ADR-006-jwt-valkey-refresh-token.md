...
- Consumer `/auth/**` 공개 SecurityFilterChain은 선택적 Authorization을 인증하지 않는다. 로그아웃 controller가 원문 Authorization 헤더를 Auth service에 전달하고 service가 `JwtTokenProvider`로 직접 선택 파싱한다. 없거나 형식 오류·손상·만료된 Access는 교차 확인값 없이 현재 Refresh를 판정하며, 각각 유효한 Access·Refresh subject가 다를 때만 `AUTH_016`으로 mutation 전에 거부한다. 이 교차 확인은 로그아웃 권한의 유일한 근거나 필수 공격 방어 경계가 아니다. CSRF를 통과한 요청은 오류 여부와 무관하게 브라우저 쿠키를 만료하지만, `AUTH_016` 응답에서는 family·index·QR epoch를 변경하지 않고 서버 폐기 성공을 주장하지 않는다.

...
- pending Set의 현재 크기는 민감 식별자 없이 `RefreshTokenRiskEventPendingCount` 지표로 관측한다. 이 값은 전달 가능한 이벤트 수가 아니라 stale member를 포함한 인덱스 멤버 수이며, 지속적으로 증가하면 전달 정체 알람과 함께 원인을 점검한다. marker는 최초 생성 시점부터 7일 절대 TTL을 적용하며, 같은 사건 재사용으로 발생 횟수를 누적해도 TTL을 연장하지 않는다. marker가 전달 뒤 삭제돼도 별도 occurrence counter는 Refresh Token family의 **실제** 만료 시각까지 남아 다음 marker를 이전 횟수보다 큰 값으로 생성한다. 전진 배포는 기존 backend writer를 먼저 정지한 뒤 MySQL, 기존 counter, 현재 pending marker의 유효 occurrence count 중 큰 값을 counter에 이관한다. family가 만료됐으면 counter는 만들지 않거나 삭제한다. 이관 실패는 새 backend를 시작하지 않고 배포 실패로 처리한다. 1시간 이상 남아 있는 marker가 현재 전달 배치에 하나라도 있으면 `RefreshTokenRiskEventMarkerLongStay=1` 존재 신호를 기록하고 5분 합계 알람으로 관측한다. 이 지표는 전체 장기 정체 건수가 아니다.
...
- 현재 ACTIVE Consumer Refresh와 요청 원문 hash가 정확히 일치할 때만 family 폐기·account index 제거·generation/token marker·QR counter 증가를 한 Lua에서 확정한다. Lua는 key type, 필수 field, 숫자 형식·overflow, family와 ACTIVE account index의 절대 만료 시각 존재와 `index expire-at >= family expire-at`을 쓰기 전에 검사하며 선형화 뒤 실패 가능한 명령을 두지 않는다. 같은 자격의 재요청은 `ALREADY_APPLIED`로 수렴한다.
- Logout coordinator는 Lua의 정상 결과를 `event=qr_epoch_logout_result outcome=applied`, `outcome=already_applied`, `outcome=not_authorized` INFO 로그로 구분한다. account·family·token 식별자, 원문 token, Valkey key와 generation은 기록하지 않으며, 이 관측은 `APPLIED`·`ALREADY_APPLIED`·cleanup-only `NOT_AUTHORIZED` 결과나 HTTP 응답을 바꾸지 않는다.
...
- Consumer `/auth/**` 공개 SecurityFilterChain은 선택적 Authorization을 인증하지 않는다. 현재 ACTIVE Refresh가 로그아웃 mutation의 유일한 권한이며, controller가 전달한 원문 Authorization은 Auth service가 감사 보조값으로만 선택 파싱한다. 정확한 Bearer의 Access 파싱 중 발생한 모든 `ServiceException`은 Access 부재로 취급하지만 예상하지 못한 `RuntimeException`은 숨기지 않는다. 각각 유효한 Access·Refresh subject가 달라도 민감 식별자 없는 `consumer_logout_access_refresh_subject_mismatch` event만 남기고 Refresh 기반 mutation을 계속한다.
- 회전 응답 유실 뒤 구 Refresh R1으로 로그아웃해도 현재 ACTIVE R2 family를 역추적해 폐기하지 않는다. 이미 회전·폐기됐거나 family가 없는 Refresh는 cleanup-only이며 QR epoch를 증가시키지 않는다. family가 없으면 account index도 조회·변경하지 않으므로 dangling member는 index의 기존 TTL까지 남을 수 있다. 이 제한을 넓히려면 stale credential이 다른 현재 로그인과 계정 QR을 무효화할 수 있는 별도 Auth 계약 승인이 필요하다.
- generation 누락·오류, Valkey unavailable/corruption, Lua null·unexpected·overflow는 `COMMON_012`로 실패 폐쇄한다. ACTIVE family의 account index 누락은 `qr_epoch_refresh_index_mismatch expected=present`, 정확한 완료 marker와 함께 남은 stale index member는 같은 event의 `expected=absent`로 기록한다. 이 로그에는 account·family·token 식별자, 원문 token, Valkey key, generation을 넣지 않는다. 일반 stateless Access 검증은 QR epoch를 조회하지 않는다.
- 위 index mismatch 또는 TTL 손상 신호가 발생하면 운영자는 제한된 접근으로 key type·PTTL·membership·family status를 확인하고 증거를 보존한 뒤 영향 경로를 격리하고 Auth 소유자에게 에스컬레이션한다. 승인된 원자 repair 절차가 없는 상태에서 `SADD`, `DEL`, `HSET` 같은 개별 수동 변경으로 복구를 추정하지 않는다.
- TTL 없는 QR epoch hash와 Refresh 보안 상태의 정확성을 위해 Auth Valkey는 `maxmemory-policy noeviction`을 사용한다. staging·production과 표준 local compose는 이를 명시하고, 외부 관리형 Valkey도 배포 전에 같은 값을 확인한다. opt-in load-test override는 비운영 환경에서 Valkey 기본 `noeviction`에 의존하므로 실행 시 runtime 설정을 별도로 확인한다.
...
- `maxmemory-policy noeviction`을 유지하려면 쓰기 실패 전에 용량 압박을 확인해야 한다. staging host의 배포 소유 systemd timer가 60초마다 인증된 `INFO memory`의 `used_memory`·`maxmemory`와 계산한 사용률을 CloudWatch에 게시한다.
- 수집·파싱·게시 실패는 정상 0%로 대체하지 않고 별도 실패 지표로 남긴다. 계정·family·token·Valkey key·비밀번호와 `INFO memory` 전체 출력은 기록하지 않는다.
- 사용률 경보 임계값과 증설 기준은 staging의 peak·p95·증가 추세를 관측한 뒤 별도 승인으로 확정한다. 이 관측은 Valkey eviction 정책, Refresh Token API 계약, application scheduler를 변경하지 않는다.

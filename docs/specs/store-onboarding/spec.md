...
- 과거 픽업 업종 제한을 위해 사용한 `businessType(CAFE/BAKERY/OTHER)`은 화면과 기능 의미에서 제거한다. rolling 전환 동안 구 요청을 받기 위해 신청 계약에는 deprecated 선택 필드로만 남기고, 누락 시 서버가 `OTHER`를 snapshot과 Store에 저장한다. 구 backend task가 남은 동안 공식 화면은 사용자 입력 없이 고정 `OTHER`를 보내며 모든 업종의 거래 기능은 `modes`가 결정한다.
...
- 이번 expand 단계의 순방향 Flyway migration은 `stores.business_type`과 `store_onboarding_application_versions.business_type`을 nullable로 완화하되 column과 기존 check constraint를 유지한다. 신규 코드는 구 요청의 값을 보존하고 누락 요청에는 `OTHER`를 기록해 롤링 배포 중 구 코드와 rollback revision이 모든 신규 행을 읽을 수 있게 한다.
- 실제 요청 필드·내부 쓰기·column·constraint 삭제는 모든 구 task가 제거되고 rollback floor를 올린 뒤 별도 contract 변경에서 수행한다.
- `BusinessType`은 Store 생성·멱등 fingerprint·onboarding snapshot·finalizer의 혼합 버전 호환 경로에만 남긴다. 공개 응답과 기능 판정에서는 제거하며 검색 카테고리 또는 사업자등록증 업태·종목에서 호환 값을 파생하지 않는다.
...
- 신규 신청은 `businessType` 없이 접수·snapshot·자동검사·최종 Store 생성까지 완료되고 두 호환 column에는 `OTHER`가 기록된다. 구 요청의 명시값도 수용·보존하며, 요청 필드와 내부 쓰기·column은 구 task 0건 및 rollback floor 상향 전에는 제거하지 않는다.

...
| `E-002` | 매장 운영자와 매장의 활성 소속 및 명의 양도 이력 | 승인 | 2026-07-28 |
...
| `CONFIRMED` | 방문·이행 완료 확정 | `FULFILLED` | 매장 운영자 권한과 대상 매장 소속 검증 |
...
- 4번 팀원의 메뉴 홀드 도메인이 공개 `MenuHoldReleaseService`를 통해 홀드 해제와 메뉴 수량 복구를 수행한다.
...
- 수령 완료는 유효한 대상 매장 소속을 가진 매장 운영자만 `CONFIRMED`에서 `PICKED_UP`으로 전이할 수 있다.
...
- 매장 관리 명령은 중앙 `store_operator_store_affiliations`와 대상 매장 상태를 추가 검증해야 한다.
...
- `ROLE_STORE_OPERATOR`만 보유하고 대상 매장 소속이 없는 계정은 해당 매장 관리 명령을 실행할 수 없다.
...
erDiagram
    ...
    STORE_OPERATOR_ACCOUNTS ||--o{ STORE_OPERATOR_STORE_AFFILIATIONS : has
    STORES ||--o{ STORE_OPERATOR_STORE_AFFILIATIONS : grants
...
- 매장 운영자와 매장은 `store_operator_store_affiliations`로 연결한다. 계정 행이나 JWT에 관리 매장 목록을 중복 저장하지 않는다.
...
- 활성 매장 소속과 핵심 업무 중복의 논리 유일성은 E-002·E-007을 따른다.
...
## E-002 매장 운영자와 매장의 소속 관계

### 확정 구조

```text
store_operator_accounts
    1 ─── N store_operator_store_affiliations N ─── 1
                                                        stores
```

- 한 매장 운영자 계정은 여러 매장을 관리할 수 있다.
- 1차 MVP에서 한 매장은 동시에 정확히 한 명의 활성 대표 운영자만 가진다.
- 현재 관리 권한의 유일한 판단 기준은 `store_operator_store_affiliations`의 활성 소속이다.
- 소속은 `ACTIVE`, `ENDED` 상태와 `started_at`, `ended_at`을 보유해 현재 관계와 과거 이력을 함께 표현한다.
- 1차 MVP는 직원·부관리자 역할을 지원하지 않으므로 소속에 역할 열을 미리 추가하지 않는다.

### 명의 양도와 권한 변경

- 매장 명의가 양도되어도 동일한 영업 실체라면 `stores` 행과 `store_id`를 유지한다.
- 이전 운영자 소속을 `ENDED`로 종료하고 새 운영자 소속을 `ACTIVE`로 생성한다.
- 이전 소속 종료와 새 소속 생성은 같은 트랜잭션에서 처리해 활성 운영자가 없거나 둘 이상인 중간 상태를 외부에 노출하지 않는다.
- 명의 양도용 공개 API는 1차 MVP에 포함하지 않고 고도화 단계에서 별도 권한과 감사 정책을 확정한 뒤 제공한다.
- 폐업한 매장을 다른 운영자에게 넘기는 방식으로 재사용하지 않는다. 폐업과 동일 영업 실체의 명의 양도는 서로 다른 사건이다.

### 채택하지 않는 구조

- `stores.store_operator_account_id` 또는 `stores.manager_id`를 현재 권한의 원본으로 두지 않는다.
- 매장 행의 관리자 ID와 소속 관계를 동시에 권한 판단에 사용하는 이중 원본 구조를 만들지 않는다.
- 계정 행이나 JWT에 관리 매장 ID 목록을 저장하지 않는다.
- 플랫폼 운영자 계정을 매장 소유자 또는 대표 운영자로 연결하지 않는다.

### 논리 제약과 조회 규칙

- `store_operator_store_affiliations`의 각 행은 하나의 운영자 계정과 하나의 매장을 참조한다.
- 매장별 `ACTIVE` 소속은 최대 한 건이어야 한다.
- 운영자별 `ACTIVE` 소속은 여러 건일 수 있다.
- `ENDED` 소속은 `ended_at`이 필수이고 다시 `ACTIVE`로 되돌리지 않는다. 재소속은 새 행으로 기록한다.
- 매장 관리 요청은 인증된 운영자 계정, 계정 활성 상태, 대상 매장의 정확한 활성 소속과 매장 상태를 모두 검사한다.
- 인덱스와 유일성 제약의 구체적인 MySQL 구현은 4단계 공통 계약과 마이그레이션 설계에서 이 논리 제약을 만족하도록 확정한다.

### 소유권

- 1번 담당자는 `store_operator_accounts`와 운영자 인증 principal을 소유한다.
- 2번 담당자는 `stores`, `store_operator_store_affiliations`와 소속 생성·변경·종료 규칙을 소유한다.
- 다른 도메인은 소속 entity나 repository를 직접 수정하지 않고 2번 담당자가 공개한 매장 운영 권한 확인 계약을 사용한다.

### 인수 조건

- 한 운영자가 두 개 이상의 매장에 활성 소속될 수 있다.
- 한 매장에 두 번째 활성 소속을 생성하려는 요청은 거부된다.
- 명의 양도 후 이전 운영자는 즉시 매장 관리 권한을 잃고 새 운영자만 권한을 얻는다.
- 명의 양도 후에도 기존 매장, 메뉴와 예약의 `store_id`는 바뀌지 않고 이전 운영자 소속 이력을 조회할 수 있다.
- `stores`에 현재 관리자 ID를 중복 저장하거나 이를 권한 판단에 사용하지 않는다.
...
- 일반 예약 생성·취소 조정자는 3번 담당자이며 공개 Application Service를 통해 4번 담당자의 메뉴 수량 처리를 같은 트랜잭션에 참여시킨다.
...
| 운영자-매장 소속 | `ENDED`, `ended_at` | 과거 운영 주체와 권한 이력 보존 |
...
- 명의 양도 전후 거래는 당시 운영자-매장 소속 또는 운영 주체 관계 버전을 보존하고 새 운영자 정보로 과거 책임을 덮어쓰지 않는다.
...
- 1번 담당자는 계정 식별자와 개인정보 최소화 경계를, 2번 담당자는 매장·메뉴·소속 종료를 소유한다.
...
- PK 값의 연속성이나 추측 난이도를 인증·인가 수단으로 사용하지 않는다. 모든 조회·명령은 소유자, 계정 유형, 매장 소속과 자원 상태를 별도로 검증한다.
...
| 활성 매장 대표 운영자 | 매장별 활성 소속 최대 한 건 | 동시에 두 명의 대표 운영자 권한 방지 |
...
- `status`를 그대로 포함한 `UNIQUE(store_id, status)`는 과거 `ENDED` 소속을 한 건만 허용하게 되므로 사용하지 않는다.
- MySQL에서 매장별 활성 소속만 유일하게 만드는 정확한 DDL은 활성 상태에서만 `store_id`를 반환하고 종료 상태에서는 `NULL`을 반환하는 생성 열 또는 동등하게 검증된 방식으로 구현한다.
- MySQL의 `NULL` 유일성 동작, 동시 명의 양도와 이력 다건 보존은 Testcontainers MySQL 통합 테스트로 검증한다.
- 활성 사업자등록번호 귀속처럼 이력은 여러 건이지만 현재 활성 행만 하나여야 하는 제약도 같은 원칙을 적용한다.
- 애플리케이션의 사전 중복 조회만으로 활성 유일성을 보장하지 않고 DB 제약과 조건부 전이를 최종 방어선으로 사용한다.

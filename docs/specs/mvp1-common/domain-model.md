...
`REQUESTED`와 `CAPACITY_HELD`는 하나의 예약 생성 트랜잭션 안에서만 사용하는 처리 단계이며 1차 MVP의 영속 상태 enum이나 공개 API 값이 아니다. 생성 검증 실패는 전체 롤백과 도메인 오류로 반환하며 `REJECTED` 예약 행을 만들지 않는다. 사용자용 임시 선점과 만료가 없으므로 `EXPIRED` 상태·진입 경로도 1차 MVP에 두지 않는다.
...
- `REJECTED`는 생성 실패 이력을 별도 영속 거래로 보존하는 기능이 승인될 때, `EXPIRED`는 사용자용 임시 선점·TTL·만료 작업이 승인될 때만 뒤 단계 상태로 재검토한다.

### 현재 확정된 전이

| 시작 상태 | 명령·사건 | 종료 상태 | 비고 |
|---|---|---|---|
| 없음 | 예약 생성과 선택 메뉴 홀드 전체 확정 | `CONFIRMED` | 검증·수용량 확보·선택 메뉴 수량 확보를 하나의 MySQL 트랜잭션에서 처리 |
| 없음 | 생성 검증 또는 자원 확보 실패 | 없음 | 전체 롤백하고 도메인 오류를 반환하며 예약 행을 남기지 않음 |
...
- 1차 MVP 예약 상태 enum과 OpenAPI에 `REQUESTED`, `CAPACITY_HELD`, `REJECTED`, `EXPIRED`, `COMPLETED`, 결제 상태, `PAY-*`, 체크인 또는 `NO_SHOW` 상태가 없다.
...
- `CANCELLED`, `FULFILLED`에서 새로운 취소 명령을 허용하지 않는다.
...
- 1차 MVP에서 수량 복구가 실패하면 예약 취소와 메뉴 홀드 `RELEASED` 전이를 함께 롤백하므로, 복구되지 않은 수량과 `RELEASED` 홀드가 부분 커밋되지 않는다. 종결 상태를 유지한 채 별도 반환 사건을 재시도하는 모델은 후속 고도화 경계다.
...
- 사업자등록증 업태·종목은 픽업 판정에 사용하지 않는다. 픽업 기능이 비활성화된 매장은 픽업 예약을 확정할 수 없지만 일반 매장·예약 기능은 영향을 받지 않는다.
...
- `InventoryAvailabilityStatus.SOLD_OUT`은 메뉴 전체 상태가 아니라 운영자가 `menuId + serviceDate + [startTime, endTime) + policyVersion`으로 식별하는 홀드·픽업 제공 구간에 설정하는 수동 차단 상태다.
- 두 `SOLD_OUT`은 문자열이 같아도 저장 위치·소유자·판정 단위와 효과가 다른 별도 타입이며 하나의 enum이나 열로 합치지 않는다.
- 중앙 온라인 가용량이 0이면 `availabilityStatus`를 변경하지 않고 계산 결과로 해당 구간의 신규 거래를 차단한다. 유효한 복구·증가로 온라인 가용량이 양수가 되어도 수동 `InventoryAvailabilityStatus.SOLD_OUT`을 자동 해제하지 않으며, 신규 거래는 최종 구간 상태가 `AVAILABLE`이고 온라인 가용량이 양수일 때만 허용한다. 수량 복구가 메뉴 전체 수동 `SOLD_OUT`·`PAUSED`·`HIDDEN`도 자동 해제하지 않는다.
...
| 픽업 기능 | `pickupEnabled=true/false` | 모든 승인 매장이 선택 가능 |

### 입점 확정

- 신규 입점은 사업자등록증과 구조화 정보의 자동검사를 먼저 통과해야 하며 접수 `202 Accepted`만으로 Store를 만들거나 `APPROVED`로 보지 않는다.
- 현재 또는 폐점 매장에 이미 귀속된 사업자등록번호는 재사용하지 않는다. 영구 중복 배제와 승인 finalizer가 경합해도 Store 하나만 생성되며 기존 Store를 변경하지 않는다.
- 신청 version은 신청자 자기확약·필수 입점 약관 동의, 사업자 구조화 정보, 증빙과 접수 시점의 수동 심사 switch를 함께 고정한다.
- 자동검사 통과 뒤 switch가 꺼진 신청은 자동 승인하고, 켜진 신청은 배정된 플랫폼 운영자의 승인 전까지 Store를 만들지 않는다. 승인된 두 경로는 같은 finalizer를 사용한다.
- 이미 승인된 기존 매장은 소급 재심사하거나 사업자등록증 재제출을 요구하지 않는다.
...
- 사업자등록증 업태·종목과 무관하게 모든 승인 매장은 픽업 기능을 활성화할 수 있다.
- 사업자등록증 업태·종목과 검색 카테고리·태그는 픽업 기능을 만들거나 제한하지 않는다. 과거 픽업 판정용 `businessType`은 화면·기능 판정에서 제거하고 rolling 호환 필드로만 임시 유지한다.
- 입점 확정과 픽업 기능은 독립적이므로 `APPROVED + pickupEnabled=false` 매장도 일반 매장·예약 기능을 사용할 수 있다.
- 픽업 예약 신규 생성은 사업자등록증 업태·종목과 무관하게 `APPROVED + OPEN + pickupEnabled`를 모두 만족하고 영업·픽업 제공 구간과 메뉴 수량 검증까지 통과해야 한다.
...
- 신청 상태는 Store 상태와 분리하며 승인 전 Store 응답에 수동 심사 상태를 끼워 넣지 않는다.
...
- `HOLD-010`의 실제 차감 원장 기준 원래 풀 복구와 `HOLD-013`의 추가 전용 감사 원장을 유지한다. 1차 MVP는 거래 상태 전이·복구 원장·수량 변경을 같은 트랜잭션에서 처리하고, 별도 반환 사건 상태는 후속 고도화에서만 활성화한다.
...
- 온라인 가용량이 0이면 `availabilityStatus`를 변경하지 않고 계산 결과로 해당 메뉴·제공 구간의 신규 홀드와 픽업을 차단한다. 다른 구간이나 `ONSITE` 수량은 변경하지 않는다.
- `InventoryAvailabilityStatus.SOLD_OUT`은 운영자가 설정한 제공 구간별 수동 차단 상태이며, 신규 홀드와 픽업만 차단하고 기존 확정 거래를 자동 취소하거나 다른 메뉴로 바꾸지 않는다.
- 실제 수량이 복구되어 온라인 가용성이 생겨도 수동 `InventoryAvailabilityStatus.SOLD_OUT`을 자동 해제하지 않는다. 최종 구간 상태가 `AVAILABLE`이고 온라인 가용량이 양수인지와 판매·공개·마감 상태를 별도로 검증한 뒤 신규 거래 가능 여부를 판정한다.

### 소유권과 미확정 물리 제약

- 4번 담당자의 메뉴 홀드·픽업 도메인이 메뉴 재고 버킷, 수량 풀, 원장, 차감·복구·제공 구간별 수동 차단 상태를 소유한다.
...
- 멱등 기록에는 HTTP method, 정규화된 route, path parameter, 승인된 query와 요청 본문을 결합한 정규화 지문, 처리 상태, 생성된 거래 식별자와 최초 성공 결과를 연결한다.
- 같은 주체·명령·멱등 키와 같은 전체 요청 지문의 재전송은 자원을 다시 변경하지 않고 최초 HTTP 상태·응답 코드·정규화 결과 payload를 반환한다.
- 같은 멱등 키를 다른 요청 지문에 재사용하면 새 명령으로 처리하지 않고 충돌로 거부한다.
- 성공 커밋 뒤 응답이 유실되거나 결과 리소스의 현재 상태가 바뀌어도 재시도는 저장된 최초 결과를 반환하고 새 예약·홀드·픽업이나 추가 차감·복구를 만들지 않는다.
...
| 예약 | `CANCELLED`, `FULFILLED` | 종결 상태 행과 수용량 배정 이력 보존 |
...
| 사업자등록번호 영구 귀속 | 정규화 사업자등록번호의 전체 매장 이력 | 폐점 후 재사용을 포함한 중복 매장 등록 방지 |
...
- 사업자등록번호는 `stores`의 운영 상태와 무관한 일반 유일 제약으로 영구 귀속을 보장한다. 현재 활성 행만 하나여야 하는 별도 이력형 제약은 상태 조건과 DB 제약을 함께 사용한다.

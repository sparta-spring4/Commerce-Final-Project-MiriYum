...
| 상태 축 | 값 | 의미 |
|---|---|---|
| 입점 검증 | `APPROVED` | 사업자등록번호 형식과 활성 중복 검사를 통과한 1차 MVP 등록 결과 |
| 매장 운영 | `OPEN`, `TEMPORARILY_CLOSED`, `CLOSED` | 영업, 휴점, 폐점의 중앙 운영 상태 |
| 픽업 자격 | `ELIGIBLE`, `INELIGIBLE` | 등록 업종 `CAFE`, `BAKERY`는 픽업 가능, `OTHER`는 픽업 불가 |

### 입점 확정

- 사업자등록번호 형식과 활성 중복 검사를 통과한 등록 명령은 플랫폼 운영자 승인 없이 즉시 `APPROVED`로 확정한다.
- 활성 매장에 이미 귀속된 사업자등록번호는 새 매장을 만들거나 실패 상태를 저장하지 않고 `STORE_002`로 거절한다.

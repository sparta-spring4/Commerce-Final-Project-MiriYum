...
- 사업자등록번호 형식과 전체 매장 이력의 영구 유일성 검사를 통과한 등록 명령은 플랫폼 운영자 승인 없이 즉시 `APPROVED`로 확정한다.
- 현재 또는 폐점 매장에 이미 귀속된 사업자등록번호는 재사용하지 않으며, 새 매장을 만들거나 실패 상태를 저장하지 않고 `STORE_002`로 거절한다.
- 등록 명령은 신청자 자기확약과 필수 입점 약관 동의를 모두 요구한다. 서버는 `applicant_self_attested_at`, `required_terms_agreed_at`, `required_terms_version`을 기록하며 현재 필수 약관 버전은 `STORE_ONBOARDING_REQUIRED_TERMS_V1`이다.
- V10 이전 매장 행은 스키마 이행을 위해 `created_at`과 현재 약관 버전으로 위 증거 열을 backfill한다. 이 값은 기존 신청자의 법적 재동의를 나타내지 않는다.
- 외부 조회, 사전 사업자 진위·업종 기준 데이터 비교, 사업자등록증 이미지, S3와 플랫폼 운영자의 승인·보완·반려 상태는 1차 MVP에 넣지 않는다.

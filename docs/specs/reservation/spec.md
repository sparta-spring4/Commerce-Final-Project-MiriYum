...
- 커밋 뒤 `PaymentService.requestRefund(...)`를 호출한다. Payment 환불 원장이 실제 금전 결과의 유일한 원본이고 obligation은 요청 의무만 나타낸다. `REQUESTED`·`VALIDATING`·`PROCESSING`, `COMMON_008`·`COMMON_012` `ServiceException`, 그 밖의 일시적 기술 예외만 같은 의무로 재대기한다. 동일 멱등 키의 명시적 `FAILED`와 그 밖의 Payment 업무 `ServiceException`은 새 환불을 만들지 않고 obligation `RECONCILIATION_REQUIRED`, process `RECOVERY_REQUIRED`로 즉시 격리한다. 결과 불명확은 obligation을 `RECONCILIATION_REQUIRED`로 예약하되 process는 `COMPENSATING`으로 유지하고, 이후 claim은 `PaymentService.reconcileRefundResult(...)`를 통해 취소 POST 없이 provider GET만 수행한다. REQUEST poll·기술 재시도와 분리된 provider GET 3회 한도에서 결과가 수렴하지 않을 때만 process를 `RECOVERY_REQUIRED`로 전환하고 수동 복구 outbox를 정확히 한 번 생성한다. QUERY operation과 대사 시도 횟수는 obligation에 영속화해 lease 만료 재claim도 REQUEST로 되돌아가지 않는다.

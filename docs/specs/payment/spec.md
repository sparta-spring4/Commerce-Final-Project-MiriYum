...
| PortOne Webhook | `POST /api/v1/payments/webhooks/portone` | `miriyum.payment.enabled=true`과 `miriyum.payment.portone.webhook-enabled=true`일 때만 등록, Access JWT 없음, Webhook signature·원문 body 검증 |

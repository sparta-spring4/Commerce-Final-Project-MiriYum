...
- 가입·로그인·재발급·CSRF 준비 4개 요청은 IP 기준 고정 윈도우로 요청을 제한한다(기본 60초당 10회, `miriyum.rate-limit.*` 프로퍼티로 조정 가능). 로그아웃과 마이페이지 등 나머지 요청은 대상이 아니다. 이 한도값은 팀이 확정한 보안 정책 수치가 아니라 개발 중 정한 임시 기본값이며, 실제 운영 기준이 정해지면 다시 조정해야 한다. 알려진 한계 3가지: (1) 서버 1대 기준 메모리 카운터라 다중 인스턴스 배포 시 인스턴스별로 따로 세어 실제로는 더 많은 요청이 허용될 수 있다, (2) 클라이언트 IP를 `getRemoteAddr()`로만 판단해 리버스 프록시·로드밸런서 뒤에서는 프록시 IP로 뭉뚱그려질 수 있다(배포 구조가 정해지면 `X-Forwarded-For` 등을 반영해야 한다), (3) 만료된 카운터 항목을 정리하지 않아 서버를 오래 띄우면 메모리가 계속 늘어난다(개발 단계 재시작 주기에서는 무시할 만한 수준). 세 한계 모두 고도화에서 Valkey 기반 분산 카운터(TTL 자동 정리 포함)로 교체하면 해결된다 — Valkey 자체는 이미 Refresh Token 상태 관리용으로 고도화 도입이 확정돼 있으므로 새 인프라를 추가하는 게 아니라 기존 계획된 인프라를 앞당겨 재사용하는 것이다.

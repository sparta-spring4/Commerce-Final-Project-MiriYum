...
- 가입·로그인·재발급·CSRF 준비 4개 요청은 IP 기준 고정 윈도우로 요청을 제한한다(기본 10분당 5회, `miriyum.rate-limit.*` 프로퍼티로 조정 가능). 로그아웃과 마이페이지 등 나머지 요청은 대상이 아니다. 이 한도값은 `docs/service-policies/18-scale-reliability.md`의 `SCALE-005`("로그인·인증번호 요청은 10분당 5회")를 그대로 따른 확정 수치다. 알려진 한계 3가지: (1) `SCALE-005`는 1·2차 MVP에서도 속도 제한을 로컬 메모리만으로 최종 판정하지 않도록 요구하지만 이 구현은 서버 1대 기준 메모리 카운터라 다중 인스턴스 배포 시 인스턴스별로 따로 세어 실제로는 더 많은 요청이 허용될 수 있다(MySQL 기반 저장소 전환은 이슈 #61에서 트래킹), (2) 클라이언트 IP를 `getRemoteAddr()`로만 판단해 리버스 프록시·로드밸런서 뒤에서는 프록시 IP로 뭉뚱그려질 수 있다(배포 구조가 정해지면 `X-Forwarded-For` 등을 반영해야 한다), (3) 만료된 카운터 항목을 정리하지 않아 서버를 오래 띄우면 메모리가 계속 늘어난다(개발 단계 재시작 주기에서는 무시할 만한 수준).

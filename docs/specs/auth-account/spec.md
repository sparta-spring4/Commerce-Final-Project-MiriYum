...
- 1차 MVP의 일반 사용자 가입은 이메일·휴대전화 소유 인증을 수행하지 않는다. 사용자가 직접 입력한 값을 `MVP 신뢰 연락처`(실제 소유 인증은 생략했지만 1차 MVP에서 사용할 수 있다고 간주한 연락처)로 저장하되, 실제 인증 완료라고 표시하지 않는다.
- 일반 사용자 휴대전화는 공백과 하이픈을 제거한 `010` 시작 11자리 숫자로 정규화한다. 다른 형식은 임의 변환하지 않고 `COMMON_001`로 거절하며, 정규화 값은 일반 사용자 테이블에서 유일해야 한다.
- 일반 사용자 가입자는 만 14세 이상임을 직접 확인한 값을 제출해야 한다. 이 값은 실제 연령 인증이 아니며 생년월일 원문은 수집하지 않는다.
- 운영 공개 전에는 일반 사용자의 이메일·휴대전화 소유와 만 14세 이상 여부를 승인된 인증 결과로 교체해야 한다.
- 매장 운영자 가입은 기존 이메일 확인과 휴대전화 본인확인 참조 계약을 유지하며 이 임시 예외를 적용하지 않는다.
...
매장 운영자 가입은 기존 `emailVerificationReference`, `identityVerificationReference` 공급자 중립 참조 계약을 유지한다. 제공업체 토큰 구조, callback payload(외부 업체가 되돌려주는 응답 본문)와 비밀 키를 MiriYum 공개 API로 노출하지 않으며, 승인된 어댑터가 연결되지 않은 운영 환경에서는 확인을 우회해 계정을 생성하지 않는다.

`miriyum.identity-verification.dev-stub-enabled`는 로컬 개발 환경에서 매장 운영자 인증 플로우를 확인하기 위한 기존 개발용 예외로만 유지한다. 실제 본인확인 어댑터가 연결되면 제거하며, 1차 MVP 일반 사용자의 신뢰 연락처 정책을 이 설정에 의존시키지 않는다.
...
- 일반 사용자 가입은 정규화 연락처·만 14세 이상 자기 확인값·계정 생성·무작위 알림 대상 참조 생성을 하나의 트랜잭션에서 확정한다. 매장 운영자 가입은 기존 확인 참조 소비와 계정 생성을 하나의 트랜잭션에서 확정한다.
...
- 일반 사용자는 정규화된 신뢰 연락처와 만 14세 이상 자기 확인값 없이 활성 계정이 생성되지 않고, 매장 운영자는 기존 확인 성공 참조 없이 활성 계정이 생성되지 않는다.
...
| 날짜 | 결정 | 선택 이유 |
| --- | --- | --- |
| 2026-08-06 | 1차 MVP 일반 사용자는 직접 입력한 연락처와 만 14세 이상 자기 확인값을 임시 신뢰하고 실제 소유·연령 인증은 고도화로 이동 | 유료 인증 연동 없이 예약 흐름을 완성하되 실제 인증 완료로 오해하지 않고 운영 공개 전 교체 경계를 남김 |

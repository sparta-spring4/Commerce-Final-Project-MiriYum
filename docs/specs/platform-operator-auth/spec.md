...
- 운영자 계정 생성·정지·권한 관리 HTTP API와 통합 감사 조회: #282
...
| 세션 유휴·절대 만료, 회수 또는 버전 불일치 | 401 | `AUTH_013` |
...
`AUTH_013`은 세션이 만료·회수됐는지, 버전이 바뀌었는지를 외부에 구분하지 않는 플랫폼 운영자 세션 무효 오류다.
...
- #282는 이 원장을 삭제·수정하지 않고 통합 감사 조회 계약에 연결한다.

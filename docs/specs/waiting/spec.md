...
> Issue #250 소비자·매장 운영자 Waiting SSE 변경 신호
>
...
`PENDING`/`PUBLISHED`는 IN_APP dispatcher 진행 상태만 나타낸다. 아래 SSE 계약과 후속
Runtime은 immutable 사건 high-watermark를 독립적으로 소비하며 이 publication state를 SSE
소비 완료로 재사용하지 않는다. AUTO 접수 오픈 worker와 settings version CAS는 Issue #380이
별도로 소유한다.

### Waiting SSE 변경 신호 계약 (#250)

| audience | endpoint | 업무 event | 수신 뒤 재조회 |
|---|---|---|---|
...
- `id`는 audience·인증 계정·필요한 store scope·계약 version에 결속한 무결성 보호 opaque cursor다. 형식·무결성이 잘못됐거나 다른 audience·계정·store cursor면 `400 COMMON_001` JSON 오류 envelope로 거절한다.

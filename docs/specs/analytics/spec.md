...
현재 `dev`의 마지막 migration은 #382로 병합된 V53이다. 가장 나중에 병합할 #270은 사용자 지정 규칙인 최신 `dev` migration 최댓값의 `+1`에 따라 `V54__create_dashboard_analytics_snapshots.sql`을 사용한다. #270 병합 직전에 다시 최신 최댓값의 `+1`인지 확인하고 전체 순서를 검증한다.
...
| V54 | 최신 dev에 Store sanctions V53이 병합되어 현재 최댓값 + 1 | Issue #270 병합 직전 다시 재확인 |

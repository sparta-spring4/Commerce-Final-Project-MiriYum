...
- 노쇼 소비를 위한 내부 계약은 최종 Reservation에 연결된 홀드를 `FORFEITED`로 종결하며 수량 복구·반환 원장·자동 승계를 만들지 않는다. Reservation `NO_SHOW` 전이, QR/API, 감사와 금액 처리는 Issue #240 이후 범위다.
...
- DB CHECK는 기존 행과 임시 행을 구분한다. 기존 행은 `reservationHoldId/expiresAt = NULL`, `reservationId IS NOT NULL`, 상태 `CONFIRMED|RELEASED|FULFILLED|FORFEITED`만 허용한다. 최종 Reservation에 연결되지 않은 임시 행은 `reservationHoldId/expiresAt IS NOT NULL`, `reservationId IS NULL`, 상태 `ACTIVE|RECONCILIATION_REQUIRED|RELEASED|EXPIRED`만 허용한다. 연결된 임시 행은 세 식별·시각 필드가 모두 존재하고 상태 `CONFIRMED|RELEASED|FULFILLED|FORFEITED`만 허용한다. V48은 이 CHECK만 교체하며 backfill은 수행하지 않는다.
...
- `ACTIVE`와 `RECONCILIATION_REQUIRED`는 수량을 유지한다. `CONFIRMED`는 수량을 유지하고 양의 최종 `reservationId`를 연결한다. 같은 확정 명령 replay는 같은 연결만 허용하며 다른 Reservation ID는 `COMMON_007`이다. 최종 Reservation에 연결된 임시 MenuHold는 이후 기존 예약 취소에서 `RELEASED`로 전이하며 최초 확보 원장을 기준으로 수량을 한 번만 복구하고, 방문 완료에서는 수량 복구 없이 `FULFILLED`로, 노쇼에서는 수량 복구 없이 `FORFEITED`로 전이한다. 최초 생성 명령 replay는 이 합법적인 후속 `RELEASED|FULFILLED|FORFEITED` 상태와 최종 Reservation 연결을 최신 결과로 반환하며 추가 수량 변경을 만들지 않는다. 같은 확정 명령의 전이 replay도 최종 연결 ID가 같으면 이 후속 종결 상태를 허용하고, reconciliation replay는 이후 확정과 최종 예약 종결까지 진행된 최신 상태로 수렴한다. 최종 Reservation에 연결되지 않은 임시 상태는 이 경계에 합류하지 않는다. `RELEASED`·`FULFILLED`·`FORFEITED`는 서로 교차 전이하지 않으며 하나만 확정된다. 기존 즉시 확정 MenuHold의 취소·이행·조회 의미와 응답은 바뀌지 않는다.
- #267의 worker·명령 시점 자동 만료·대사 orchestration과 #238의 Payment/PG 판정·최종 Reservation 생성은 이 단계에서 구현하지 않는다.
- `RECONCILIATION_REQUIRED`는 자동 만료하지 않고 수용량과 메뉴 수량을 유지한다. 이 상태로 전이한 감사의 `occurredAt`부터 10분이 지나면 장기 체류로 판정한다. #266은 수렴 주체를 만들지 않으며, #267 runtime을 활성화하기 전에는 현재 장기 체류 그룹의 존재를 식별자 없이 관측하는 지표·알람과 replay-safe confirm/release 수렴 절차를 함께 제공해야 한다. 지표·알람 관측은 상태나 수량을 변경하지 않으며 자동 만료·해제 근거가 아니다.
...
- 사업자등록증 업태·종목·검색 카테고리·태그·요청 본문의 임의 업종 값은 픽업 가능 판정에 사용하지 않는다.
...
- 사업자등록증 업태·종목은 픽업 가능 판정에 참여하지 않으며, 픽업 기능 상태는 일반 예약 기능에 영향을 주지 않는다.

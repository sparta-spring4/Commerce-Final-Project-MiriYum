...
신규 migration은 반드시 `backend/src/main/resources/db/migration/V46__create_member_support.sql` 하나다. 최신 `dev`가 파일 메타데이터 V42, 운영자 관리·감사 V43, 대기 예약 V44·V45를 소유하므로 다음 빈 버전 V46을 사용하며 기존 migration을 수정하지 않는다. 복구·제재·이의·추가 승인·감사·mock 확인 session·대상 guard 원장을 만들고 `consumer_accounts`, `store_operator_accounts`에 `password_reset_required`, `support_version`을 추가한다. 연락처·인증 비밀 원문은 신규 테이블에 저장하지 않는다.
...
- `backend/src/main/resources/db/migration/V46__create_member_support.sql`
...
1. 계약·정책 테스트에서 OpenAPI path, 오류, 권한 catalog와 migration V46을 먼저 실패시킨다.

...
- 교체 중 새 파일 저장 또는 검증이 실패하면 새 파일은 공개하지 않고 기존 공개 이미지를 유지한다. 새 객체는 먼저 `PENDING`으로 저장하고, 바깥 명령과 멱등 결과가 같은 트랜잭션에서 커밋될 때만 `CONFIRMED`로 전이한다. 바깥 트랜잭션이 롤백되면 `PENDING` 파일을 `DELETED`로 보상 전이하고 물리 삭제를 시도한다. 메타데이터 전이가 실패하면 파일은 `PENDING`으로, 물리 삭제만 실패하면 `DELETED`로 남아 모두 공개 조회 대상에서 제외된다. 실패 로그의 `fileId`를 사용한 공통 저장소 정리 재호출로 물리 삭제를 다시 시도한다. 기존 파일의 물리 삭제는 새 결과와 멱등 기록이 커밋된 뒤에만 시작하며, 삭제 실패는 응답을 바꾸지 않고 같은 멱등 키 재시도에서 다시 정리한다. 이 재시도는 메타데이터 전이가 실패해 `CONFIRMED`로 남은 경우와 객체 삭제만 실패해 `DELETED`로 남은 경우 모두 처리한다. 삭제는 이미 삭제된 이미지에도 같은 종료 상태로 수렴해야 한다.

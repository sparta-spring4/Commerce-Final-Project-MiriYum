...
2. 관련도는 남은 키워드의 `매장명 정확 일치 → 매장명 포함 → 게시 메뉴명 포함 → 지역·주소 포함 → 구조화 조건만 일치`의 고정 tier로 계산하고, 같은 tier는 매장명 오름차순과 매장 ID 오름차순으로 정렬한다. 게시 메뉴명 tier는 남은 문장 전체가 현재 공개 메뉴명에 포함되는 기존 정방향 조건과, 현재 공개 메뉴명이 남은 문장에 명시된 제한적 역방향 조건을 함께 사용한다. 제한적 역방향은 trim 메뉴명 2자 이상에만 적용하고 `면`, `탕`, `국`, `밥`, `메뉴`, `음식`, `요리`, `식사`, `세트`, `정식`, `음료`를 제외한다. 같은 원문에서 후보 메뉴명이 다른 더 긴 현재 게시 메뉴명에 포함되면 더 구체적인 긴 이름만 인정해 짧은 부분 메뉴명으로 후퇴하지 않는다. 임의 실수 가중치나 개인 이력은 사용하지 않는다.

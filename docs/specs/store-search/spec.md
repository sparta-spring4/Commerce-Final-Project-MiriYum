...
- 교체 중 새 파일 저장 또는 검증이 실패하면 새 파일은 공개하지 않고 기존 공개 이미지를 유지한다. 새 파일 메타데이터는 공통 저장소의 독립 트랜잭션에서 먼저 `CONFIRMED`가 될 수 있으므로, 바깥 멱등 처리 또는 명령 트랜잭션이 롤백되면 새 파일을 `DELETED`로 보상 전이하고 물리 삭제를 시도한다. 기존 파일의 물리 삭제는 새 결과와 멱등 기록이 커밋된 뒤에만 시작하며, 삭제 실패는 응답을 바꾸지 않고 같은 멱등 키 재시도에서 다시 정리한다. 삭제는 이미 삭제된 이미지에도 같은 종료 상태로 수렴해야 한다.

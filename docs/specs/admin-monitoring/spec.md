...
- MenuHold는 V62에서 source-owned `status_version`과 append-only 전이 원장을 추가한다.
- V62 기존 행은 migration 시각의 `BASELINE` 하나만 기록한다. 그 이전 `asOf`는 `UNAVAILABLE`이며 과거 전이를 추측하지 않는다.

...
서비스 간 재고 이전은 향후 결정할 사항이다. 이전 정책이 확정되기 전에는 이전 버튼, 이전 라우트, 이전 상태, 수량 한도, 감사 이력, API, placeholder 흐름을 만들지 않는다. 현재의 공유 풀 계약은 이 독립 경계와 충돌하는 증거이므로, 일반 예약 메뉴 홀드와 분리된 픽업 재고 지원 또는 재고 이전을 활성화하는 근거로 인정하지 않는다.
...
결제·환불과 결제 제공자·Webhook 동기화, 알림, 웨이팅과 실시간 전달, 취소표 자동 승계, 웨이팅 기반 거리 추천, 체크인·노쇼, 카카오 소셜 로그인·세션, 관련 이력 모듈도 고도화라는 이름이 아니라 같은 활성화 공식으로 판단한다. 각 기능은 서로 독립된 계약, Controller·Service 실제 구현, 호출 가능 증거, 현재 배포 승인을 확인한다. 특히 2차 MVP 대체 메뉴의 3km 규칙을 웨이팅 기반 거리 추천에 전용하지 않고, 취소표 자동 승계의 순서·제한·상태를 클라이언트에서 발명하지 않는다. 증거가 부족하면 해당 내비게이션, 라우트 등록, 탭, 버튼, 데이터 요청, 액션을 만들거나 프로덕션 번들에 포함하지 않으며 준비 중 화면, 비활성 버튼, 가짜 성공, 프로덕션 mock으로 대체하지 않는다.

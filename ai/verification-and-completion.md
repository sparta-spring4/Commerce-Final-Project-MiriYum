...
계약 선행 작업은 공개 Service 메서드·DTO·오류와 해당 테스트가 실제로 동작하고 `dev`에 병합됐을 때만 소비 가능하다고 판정한다. production `return null`, 가짜 성공 응답, 빈 구현과 `UnsupportedOperationException`은 통과 가능한 계약 증거가 아니다. 선행 계약이 없으면 관련 소비 작업과 통합 검증은 `BLOCKED`다.

...
|---|---|---|---|
| backend-only | 현재는 이 문서와 관련 `docs/` 정본 | `backend/AGENTS.md`에서 선택하는 엔드 문서 | backend 스캐폴드와 로컬 AI 계약이 실제 생성되고 검증됨 |
| frontend-only | 현재는 이 문서와 관련 `docs/` 정본 | `frontend/AGENTS.md`에서 선택하는 엔드 문서 | frontend 스캐폴드와 로컬 AI 계약이 실제 생성되고 검증됨 |
...
Phase 2 전에는 `backend/AGENTS.md`와 `frontend/AGENTS.md`를 포함한 엔드 경로 및 runtime을 모두 `NOT CONFIGURED`로 취급한다. 경로, 명령, command ID가 존재한다고 추론하지 않는다. Phase 2 이후 backend-only는 `backend/AGENTS.md -> backend/ai/document-routing.md`, frontend-only는 `frontend/AGENTS.md -> frontend/ai/document-routing.md`, cross-end는 `ai/integration-contracts.md`와 필요한 엔드 문서로 라우팅한다.
...
## Phase 2 backend·frontend 스캐폴딩 계획 연결

다음 단계는 실제 backend·frontend 스캐폴드, 각 엔드의 `AGENTS.md`와 로컬 AI 계약, wrapper/package script 및 성공·실패 실행 증거를 원자적으로 만든다. 그 증거가 있을 때만 엔드별 문서와 루트 command registry를 활성화한다.

...
계획, 규범 문서, 기능 결정, 코드 변경, 검토(review), 검증 또는 완료 작업에 사용한다. 시작 순서는 `AGENTS → routing → Issue → feature spec → OpenAPI → 필요한 공통 계약·소유권·상태 전이 → 엔드 가드레일 → CONTRIBUTING`이다. 단계 5 확정 이후 저장소 쓰기 작업은 반드시 Issue에서 시작한다.

Issue에서 구현 담당자, 도메인 소유자, 정확한 포함·제외 경로, 인수 조건, 검증 계획과 필요한 `contract-first`·`blocks`·`blocked by` 의존 관계를 확인한다. 이 항목 중 적용 가능한 값이 누락됐다면 작업을 시작하지 않는다. 기능(feature) 및 주제 소유 문서를 선택한 뒤 현재 단계에 필요한 문서만 추가한다.
...
와이어프레임과 화면 시안은 UI 작업에서 화면 구조와 사용자 흐름을 파악할 때만 조건부로 읽는다. 시안은 활성 정본이 아니며, 표시된 기능·수치·상태·문구·권한·경로를 구현 계약으로 복사하지 않는다. 시안과 `docs/` 정본 또는 OpenAPI가 다르면 구현을 멈추고 정본 소유자에게 차이를 보고한다.
...
다른 도메인의 공개 계약이 필요한 작업은 Issue에서 `contract-first`, `blocks`, `blocked by` 관계를 확인한다. 소유 도메인의 최소 공개 Service 메서드·DTO·오류·테스트 계약이 선행 PR로 `dev`에 병합되기 전에는 소비 측 구현을 시작하지 않는다.
